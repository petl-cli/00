...
var projectsGetProjectRecordCmd = &cobra.Command{
	Use:   "get-project-record",
	Short: "Get a project",
	RunE:  runProjectsGetProjectRecord,
}

var projectsGetProjectRecordFlags struct {
	projectGid string
	optPretty  bool
	optFields  []string
}
...
func init() {
	projectsGetProjectRecordCmd.Flags().StringVar(&projectsGetProjectRecordFlags.projectGid, "project-gid", "", "Globally unique identifier for the project.")
	projectsGetProjectRecordCmd.MarkFlagRequired("project-gid")
	projectsGetProjectRecordCmd.Flags().BoolVar(&projectsGetProjectRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	projectsGetProjectRecordCmd.Flags().StringSliceVar(&projectsGetProjectRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	projectsCmd.AddCommand(projectsGetProjectRecordCmd)
}

func runProjectsGetProjectRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["project_gid"] = fmt.Sprintf("%v", projectsGetProjectRecordFlags.projectGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", projectsGetProjectRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = projectsGetProjectRecordFlags.optFields
	}
