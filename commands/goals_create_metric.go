...
var goalsCreateMetricCmd = &cobra.Command{
	Use:   "create-metric",
	Short: "Create a goal metric",
	RunE:  runGoalsCreateMetric,
}

var goalsCreateMetricFlags struct {
	goalGid   string
	optPretty bool
	optFields []string
	data      string
...
func init() {
	goalsCreateMetricCmd.Flags().StringVar(&goalsCreateMetricFlags.goalGid, "goal-gid", "", "Globally unique identifier for the goal.")
	goalsCreateMetricCmd.MarkFlagRequired("goal-gid")
	goalsCreateMetricCmd.Flags().BoolVar(&goalsCreateMetricFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	goalsCreateMetricCmd.Flags().StringSliceVar(&goalsCreateMetricFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	goalsCreateMetricCmd.Flags().StringVar(&goalsCreateMetricFlags.data, "data", "", "")
	// Note: body fields are not MarkFlagRequired — --body JSON satisfies them too.
	goalsCreateMetricCmd.Flags().StringVar(&goalsCreateMetricFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	goalsCmd.AddCommand(goalsCreateMetricCmd)
}

func runGoalsCreateMetric(cmd *cobra.Command, args []string) error {
	...
	pathParams["goal_gid"] = fmt.Sprintf("%v", goalsCreateMetricFlags.goalGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/goals/{goal_gid}/setMetric", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", goalsCreateMetricFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = goalsCreateMetricFlags.optFields
	}
	...
	if goalsCreateMetricFlags.body != "" {
		if err := json.Unmarshal([]byte(goalsCreateMetricFlags.body), &bodyMap); err != nil {
	...
	if cmd.Flags().Changed("data") {
		bodyMap["data"] = goalsCreateMetricFlags.data
	}
	req.Body = bodyMap
