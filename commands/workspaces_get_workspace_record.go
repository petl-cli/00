...
var workspacesGetWorkspaceRecordCmd = &cobra.Command{
	Use:   "get-workspace-record",
	Short: "Get a workspace",
	RunE:  runWorkspacesGetWorkspaceRecord,
}

var workspacesGetWorkspaceRecordFlags struct {
	workspaceGid string
	optPretty    bool
	optFields    []string
}
...
func init() {
	workspacesGetWorkspaceRecordCmd.Flags().StringVar(&workspacesGetWorkspaceRecordFlags.workspaceGid, "workspace-gid", "", "Globally unique identifier for the workspace or organization.")
	workspacesGetWorkspaceRecordCmd.MarkFlagRequired("workspace-gid")
	workspacesGetWorkspaceRecordCmd.Flags().BoolVar(&workspacesGetWorkspaceRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	workspacesGetWorkspaceRecordCmd.Flags().StringSliceVar(&workspacesGetWorkspaceRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	workspacesCmd.AddCommand(workspacesGetWorkspaceRecordCmd)
}

func runWorkspacesGetWorkspaceRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["workspace_gid"] = fmt.Sprintf("%v", workspacesGetWorkspaceRecordFlags.workspaceGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", workspacesGetWorkspaceRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = workspacesGetWorkspaceRecordFlags.optFields
	}
