...
var taskTemplatesListMultipleCmd = &cobra.Command{
	Use:   "list-multiple",
	Short: "Get multiple task templates",
	RunE:  runTaskTemplatesListMultiple,
}

var taskTemplatesListMultipleFlags struct {
	optPretty bool
	limit     int
	offset    string
	project   string
...
func init() {
	taskTemplatesListMultipleCmd.Flags().BoolVar(&taskTemplatesListMultipleFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	taskTemplatesListMultipleCmd.Flags().IntVar(&taskTemplatesListMultipleFlags.limit, "limit", 0, "Results per page. The number of objects to return per page. The value must be between 1 and 100.")
	taskTemplatesListMultipleCmd.Flags().StringVar(&taskTemplatesListMultipleFlags.offset, "offset", "", "Offset token. An offset to the next page returned by the API. A pagination request will return an offset token, which can be used as an input parameter to the next request. If an offset is not passed in, the API will return the first page of results. *Note: You can only pass in an offset that was returned to you via a previously paginated request.*")
	taskTemplatesListMultipleCmd.Flags().StringVar(&taskTemplatesListMultipleFlags.project, "project", "", "The project to filter task templates on.")
	taskTemplatesListMultipleCmd.Flags().StringSliceVar(&taskTemplatesListMultipleFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	taskTemplatesCmd.AddCommand(taskTemplatesListMultipleCmd)
}

func runTaskTemplatesListMultiple(cmd *cobra.Command, args []string) error {
	...
	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/task_templates", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", taskTemplatesListMultipleFlags.optPretty)
	}
	if cmd.Flags().Changed("limit") {
		req.QueryParams["limit"] = fmt.Sprintf("%v", taskTemplatesListMultipleFlags.limit)
	}
	if cmd.Flags().Changed("offset") {
		req.QueryParams["offset"] = fmt.Sprintf("%v", taskTemplatesListMultipleFlags.offset)
	}
	if cmd.Flags().Changed("project") {
		req.QueryParams["project"] = fmt.Sprintf("%v", taskTemplatesListMultipleFlags.project)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = taskTemplatesListMultipleFlags.optFields
	}
