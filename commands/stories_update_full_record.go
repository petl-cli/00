...
var storiesUpdateFullRecordCmd = &cobra.Command{
	Use:   "update-full-record",
	Short: "Update a story",
	RunE:  runStoriesUpdateFullRecord,
}

var storiesUpdateFullRecordFlags struct {
	storyGid  string
	optPretty bool
	optFields []string
	data      string
...
func init() {
	storiesUpdateFullRecordCmd.Flags().StringVar(&storiesUpdateFullRecordFlags.storyGid, "story-gid", "", "Globally unique identifier for the story.")
	storiesUpdateFullRecordCmd.MarkFlagRequired("story-gid")
	storiesUpdateFullRecordCmd.Flags().BoolVar(&storiesUpdateFullRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	storiesUpdateFullRecordCmd.Flags().StringSliceVar(&storiesUpdateFullRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	storiesUpdateFullRecordCmd.Flags().StringVar(&storiesUpdateFullRecordFlags.data, "data", "", "")
	// Note: body fields are not MarkFlagRequired — --body JSON satisfies them too.
	storiesUpdateFullRecordCmd.Flags().StringVar(&storiesUpdateFullRecordFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	storiesCmd.AddCommand(storiesUpdateFullRecordCmd)
}

func runStoriesUpdateFullRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["story_gid"] = fmt.Sprintf("%v", storiesUpdateFullRecordFlags.storyGid)

	req := &httpclient.Request{
		Method:      "PUT",
		Path:        httpclient.SubstitutePath("/stories/{story_gid}", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", storiesUpdateFullRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = storiesUpdateFullRecordFlags.optFields
	}
	...
	if storiesUpdateFullRecordFlags.body != "" {
		if err := json.Unmarshal([]byte(storiesUpdateFullRecordFlags.body), &bodyMap); err != nil {
	...
	if cmd.Flags().Changed("data") {
		bodyMap["data"] = storiesUpdateFullRecordFlags.data
	}
	req.Body = bodyMap
