...
var goalsDeleteRecordCmd = &cobra.Command{
	Use:   "delete-record",
	Short: "Delete a goal",
	RunE:  runGoalsDeleteRecord,
}

var goalsDeleteRecordFlags struct {
	goalGid   string
	optPretty bool
}

func init() {
	goalsDeleteRecordCmd.Flags().StringVar(&goalsDeleteRecordFlags.goalGid, "goal-gid", "", "Globally unique identifier for the goal.")
	goalsDeleteRecordCmd.MarkFlagRequired("goal-gid")
	goalsDeleteRecordCmd.Flags().BoolVar(&goalsDeleteRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")

	goalsCmd.AddCommand(goalsDeleteRecordCmd)
}

func runGoalsDeleteRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["goal_gid"] = fmt.Sprintf("%v", goalsDeleteRecordFlags.goalGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", goalsDeleteRecordFlags.optPretty)
	}
