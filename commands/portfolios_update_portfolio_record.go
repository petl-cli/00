...
var portfoliosUpdatePortfolioRecordCmd = &cobra.Command{
	Use:   "update-portfolio-record",
	Short: "Update a portfolio",
	RunE:  runPortfoliosUpdatePortfolioRecord,
}

var portfoliosUpdatePortfolioRecordFlags struct {
	portfolioGid string
	optPretty    bool
	optFields    []string
	data         string
...
func init() {
	portfoliosUpdatePortfolioRecordCmd.Flags().StringVar(&portfoliosUpdatePortfolioRecordFlags.portfolioGid, "portfolio-gid", "", "Globally unique identifier for the portfolio.")
	portfoliosUpdatePortfolioRecordCmd.MarkFlagRequired("portfolio-gid")
	portfoliosUpdatePortfolioRecordCmd.Flags().BoolVar(&portfoliosUpdatePortfolioRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	portfoliosUpdatePortfolioRecordCmd.Flags().StringSliceVar(&portfoliosUpdatePortfolioRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	portfoliosUpdatePortfolioRecordCmd.Flags().StringVar(&portfoliosUpdatePortfolioRecordFlags.data, "data", "", "")
	// Note: body fields are not MarkFlagRequired — --body JSON satisfies them too.
	portfoliosUpdatePortfolioRecordCmd.Flags().StringVar(&portfoliosUpdatePortfolioRecordFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	portfoliosCmd.AddCommand(portfoliosUpdatePortfolioRecordCmd)
}

func runPortfoliosUpdatePortfolioRecord(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "update-portfolio-record",
	...
	pathParams["portfolio_gid"] = fmt.Sprintf("%v", portfoliosUpdatePortfolioRecordFlags.portfolioGid)

	req := &httpclient.Request{
		Method:      "PUT",
		Path:        httpclient.SubstitutePath("/portfolios/{portfolio_gid}", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", portfoliosUpdatePortfolioRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = portfoliosUpdatePortfolioRecordFlags.optFields
	}
	...
	if portfoliosUpdatePortfolioRecordFlags.body != "" {
		if err := json.Unmarshal([]byte(portfoliosUpdatePortfolioRecordFlags.body), &bodyMap); err != nil {
	...
	if cmd.Flags().Changed("data") {
		bodyMap["data"] = portfoliosUpdatePortfolioRecordFlags.data
	}
	req.Body = bodyMap
