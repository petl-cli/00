...
var projectTemplatesDeleteTemplateRecordCmd = &cobra.Command{
	Use:   "delete-template-record",
	Short: "Delete a project template",
	RunE:  runProjectTemplatesDeleteTemplateRecord,
}

var projectTemplatesDeleteTemplateRecordFlags struct {
	projectTemplateGid string
	optPretty          bool
}

func init() {
	projectTemplatesDeleteTemplateRecordCmd.Flags().StringVar(&projectTemplatesDeleteTemplateRecordFlags.projectTemplateGid, "project-template-gid", "", "Globally unique identifier for the project template.")
	projectTemplatesDeleteTemplateRecordCmd.MarkFlagRequired("project-template-gid")
	projectTemplatesDeleteTemplateRecordCmd.Flags().BoolVar(&projectTemplatesDeleteTemplateRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")

	projectTemplatesCmd.AddCommand(projectTemplatesDeleteTemplateRecordCmd)
}

func runProjectTemplatesDeleteTemplateRecord(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "delete-template-record",
	...
	pathParams["project_template_gid"] = fmt.Sprintf("%v", projectTemplatesDeleteTemplateRecordFlags.projectTemplateGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", projectTemplatesDeleteTemplateRecordFlags.optPretty)
	}
