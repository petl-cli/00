...
var portfolioMembershipsGetCompleteRecordCmd = &cobra.Command{
	Use:   "get-complete-record",
	Short: "Get a portfolio membership",
	RunE:  runPortfolioMembershipsGetCompleteRecord,
}

var portfolioMembershipsGetCompleteRecordFlags struct {
...
func init() {
	portfolioMembershipsGetCompleteRecordCmd.Flags().StringVar(&portfolioMembershipsGetCompleteRecordFlags.portfolioMembershipGid, "portfolio-membership-gid", "", "")
	portfolioMembershipsGetCompleteRecordCmd.MarkFlagRequired("portfolio-membership-gid")
	portfolioMembershipsGetCompleteRecordCmd.Flags().BoolVar(&portfolioMembershipsGetCompleteRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	portfolioMembershipsGetCompleteRecordCmd.Flags().StringSliceVar(&portfolioMembershipsGetCompleteRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	portfolioMembershipsCmd.AddCommand(portfolioMembershipsGetCompleteRecordCmd)
}

func runPortfolioMembershipsGetCompleteRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["portfolio_membership_gid"] = fmt.Sprintf("%v", portfolioMembershipsGetCompleteRecordFlags.portfolioMembershipGid)

	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/portfolio_memberships/{portfolio_membership_gid}", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", portfolioMembershipsGetCompleteRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = portfolioMembershipsGetCompleteRecordFlags.optFields
	}
