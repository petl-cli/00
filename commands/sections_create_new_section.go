...
var sectionsCreateNewSectionCmd = &cobra.Command{
	Use:   "create-new-section",
	Short: "Create a section in a project",
	RunE:  runSectionsCreateNewSection,
}

var sectionsCreateNewSectionFlags struct {
	projectGid string
	optPretty  bool
	optFields  []string
	body       string
...
func init() {
	sectionsCreateNewSectionCmd.Flags().StringVar(&sectionsCreateNewSectionFlags.projectGid, "project-gid", "", "Globally unique identifier for the project.")
	sectionsCreateNewSectionCmd.MarkFlagRequired("project-gid")
	sectionsCreateNewSectionCmd.Flags().BoolVar(&sectionsCreateNewSectionFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	sectionsCreateNewSectionCmd.Flags().StringSliceVar(&sectionsCreateNewSectionFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	sectionsCreateNewSectionCmd.Flags().StringVar(&sectionsCreateNewSectionFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	sectionsCmd.AddCommand(sectionsCreateNewSectionCmd)
}

func runSectionsCreateNewSection(cmd *cobra.Command, args []string) error {
	...
	pathParams["project_gid"] = fmt.Sprintf("%v", sectionsCreateNewSectionFlags.projectGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", sectionsCreateNewSectionFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = sectionsCreateNewSectionFlags.optFields
	}
	...
	if sectionsCreateNewSectionFlags.body != "" {
		if err := json.Unmarshal([]byte(sectionsCreateNewSectionFlags.body), &bodyMap); err != nil {
