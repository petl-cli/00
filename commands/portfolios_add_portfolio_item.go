...
var portfoliosAddPortfolioItemCmd = &cobra.Command{
	Use:   "add-portfolio-item",
	Short: "Add a portfolio item",
	RunE:  runPortfoliosAddPortfolioItem,
}

var portfoliosAddPortfolioItemFlags struct {
	portfolioGid string
	optPretty    bool
	body         string
}
...
func init() {
	portfoliosAddPortfolioItemCmd.Flags().StringVar(&portfoliosAddPortfolioItemFlags.portfolioGid, "portfolio-gid", "", "Globally unique identifier for the portfolio.")
	portfoliosAddPortfolioItemCmd.MarkFlagRequired("portfolio-gid")
	portfoliosAddPortfolioItemCmd.Flags().BoolVar(&portfoliosAddPortfolioItemFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	portfoliosAddPortfolioItemCmd.Flags().StringVar(&portfoliosAddPortfolioItemFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	portfoliosCmd.AddCommand(portfoliosAddPortfolioItemCmd)
}

func runPortfoliosAddPortfolioItem(cmd *cobra.Command, args []string) error {
	...
	pathParams["portfolio_gid"] = fmt.Sprintf("%v", portfoliosAddPortfolioItemFlags.portfolioGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/portfolios/{portfolio_gid}/addItem", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", portfoliosAddPortfolioItemFlags.optPretty)
	}
	...
	if portfoliosAddPortfolioItemFlags.body != "" {
		if err := json.Unmarshal([]byte(portfoliosAddPortfolioItemFlags.body), &bodyMap); err != nil {
