...
var taskTemplatesGetSingleTemplateCmd = &cobra.Command{
	Use:   "get-single-template",
	Short: "Get a task template",
	RunE:  runTaskTemplatesGetSingleTemplate,
}

var taskTemplatesGetSingleTemplateFlags struct {
...
func init() {
	taskTemplatesGetSingleTemplateCmd.Flags().StringVar(&taskTemplatesGetSingleTemplateFlags.taskTemplateGid, "task-template-gid", "", "Globally unique identifier for the task template.")
	taskTemplatesGetSingleTemplateCmd.MarkFlagRequired("task-template-gid")
	taskTemplatesGetSingleTemplateCmd.Flags().BoolVar(&taskTemplatesGetSingleTemplateFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	taskTemplatesGetSingleTemplateCmd.Flags().StringSliceVar(&taskTemplatesGetSingleTemplateFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	taskTemplatesCmd.AddCommand(taskTemplatesGetSingleTemplateCmd)
}

func runTaskTemplatesGetSingleTemplate(cmd *cobra.Command, args []string) error {
	...
	pathParams["task_template_gid"] = fmt.Sprintf("%v", taskTemplatesGetSingleTemplateFlags.taskTemplateGid)

	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/task_templates/{task_template_gid}", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", taskTemplatesGetSingleTemplateFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = taskTemplatesGetSingleTemplateFlags.optFields
	}
