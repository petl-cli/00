...
var customFieldsCreateNewFieldRecordCmd = &cobra.Command{
	Use:   "create-new-field-record",
	Short: "Create a custom field",
	RunE:  runCustomFieldsCreateNewFieldRecord,
}

var customFieldsCreateNewFieldRecordFlags struct {
	optPretty bool
	optFields []string
	data      string
	body      string
...
func init() {
	customFieldsCreateNewFieldRecordCmd.Flags().BoolVar(&customFieldsCreateNewFieldRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	customFieldsCreateNewFieldRecordCmd.Flags().StringSliceVar(&customFieldsCreateNewFieldRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	customFieldsCreateNewFieldRecordCmd.Flags().StringVar(&customFieldsCreateNewFieldRecordFlags.data, "data", "", "")
	// Note: body fields are not MarkFlagRequired — --body JSON satisfies them too.
	customFieldsCreateNewFieldRecordCmd.Flags().StringVar(&customFieldsCreateNewFieldRecordFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	customFieldsCmd.AddCommand(customFieldsCreateNewFieldRecordCmd)
}

func runCustomFieldsCreateNewFieldRecord(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "create-new-field-record",
	...
	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/custom_fields", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", customFieldsCreateNewFieldRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = customFieldsCreateNewFieldRecordFlags.optFields
	}
	...
	if customFieldsCreateNewFieldRecordFlags.body != "" {
		if err := json.Unmarshal([]byte(customFieldsCreateNewFieldRecordFlags.body), &bodyMap); err != nil {
	...
	if cmd.Flags().Changed("data") {
		bodyMap["data"] = customFieldsCreateNewFieldRecordFlags.data
	}
	req.Body = bodyMap
