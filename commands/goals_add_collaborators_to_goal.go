...
var goalsAddCollaboratorsToGoalCmd = &cobra.Command{
	Use:   "add-collaborators-to-goal",
	Short: "Add a collaborator to a goal",
	RunE:  runGoalsAddCollaboratorsToGoal,
}

var goalsAddCollaboratorsToGoalFlags struct {
	goalGid   string
	optPretty bool
	optFields []string
	body      string
...
func init() {
	goalsAddCollaboratorsToGoalCmd.Flags().StringVar(&goalsAddCollaboratorsToGoalFlags.goalGid, "goal-gid", "", "Globally unique identifier for the goal.")
	goalsAddCollaboratorsToGoalCmd.MarkFlagRequired("goal-gid")
	goalsAddCollaboratorsToGoalCmd.Flags().BoolVar(&goalsAddCollaboratorsToGoalFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	goalsAddCollaboratorsToGoalCmd.Flags().StringSliceVar(&goalsAddCollaboratorsToGoalFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	goalsAddCollaboratorsToGoalCmd.Flags().StringVar(&goalsAddCollaboratorsToGoalFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	goalsCmd.AddCommand(goalsAddCollaboratorsToGoalCmd)
}

func runGoalsAddCollaboratorsToGoal(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "add-collaborators-to-goal",
	...
	pathParams["goal_gid"] = fmt.Sprintf("%v", goalsAddCollaboratorsToGoalFlags.goalGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/goals/{goal_gid}/addFollowers", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", goalsAddCollaboratorsToGoalFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = goalsAddCollaboratorsToGoalFlags.optFields
	}
	...
	if goalsAddCollaboratorsToGoalFlags.body != "" {
		if err := json.Unmarshal([]byte(goalsAddCollaboratorsToGoalFlags.body), &bodyMap); err != nil {
