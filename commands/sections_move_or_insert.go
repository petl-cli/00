...
var sectionsMoveOrInsertCmd = &cobra.Command{
	Use:   "move-or-insert",
	Short: "Move or Insert sections",
	RunE:  runSectionsMoveOrInsert,
}

var sectionsMoveOrInsertFlags struct {
	projectGid string
	optPretty  bool
	body       string
}
...
func init() {
	sectionsMoveOrInsertCmd.Flags().StringVar(&sectionsMoveOrInsertFlags.projectGid, "project-gid", "", "Globally unique identifier for the project.")
	sectionsMoveOrInsertCmd.MarkFlagRequired("project-gid")
	sectionsMoveOrInsertCmd.Flags().BoolVar(&sectionsMoveOrInsertFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	sectionsMoveOrInsertCmd.Flags().StringVar(&sectionsMoveOrInsertFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	sectionsCmd.AddCommand(sectionsMoveOrInsertCmd)
}

func runSectionsMoveOrInsert(cmd *cobra.Command, args []string) error {
	...
	pathParams["project_gid"] = fmt.Sprintf("%v", sectionsMoveOrInsertFlags.projectGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/projects/{project_gid}/sections/insert", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", sectionsMoveOrInsertFlags.optPretty)
	}
	...
	if sectionsMoveOrInsertFlags.body != "" {
		if err := json.Unmarshal([]byte(sectionsMoveOrInsertFlags.body), &bodyMap); err != nil {
