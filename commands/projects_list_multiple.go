...
var projectsListMultipleCmd = &cobra.Command{
	Use:   "list-multiple",
	Short: "Get multiple projects",
	RunE:  runProjectsListMultiple,
}

var projectsListMultipleFlags struct {
	optPretty bool
	limit     int
	offset    string
	workspace string
...
func init() {
	projectsListMultipleCmd.Flags().BoolVar(&projectsListMultipleFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	projectsListMultipleCmd.Flags().IntVar(&projectsListMultipleFlags.limit, "limit", 0, "Results per page. The number of objects to return per page. The value must be between 1 and 100.")
	projectsListMultipleCmd.Flags().StringVar(&projectsListMultipleFlags.offset, "offset", "", "Offset token. An offset to the next page returned by the API. A pagination request will return an offset token, which can be used as an input parameter to the next request. If an offset is not passed in, the API will return the first page of results. *Note: You can only pass in an offset that was returned to you via a previously paginated request.*")
	projectsListMultipleCmd.Flags().StringVar(&projectsListMultipleFlags.workspace, "workspace", "", "The workspace or organization to filter projects on.")
	projectsListMultipleCmd.Flags().StringVar(&projectsListMultipleFlags.team, "team", "", "The team to filter projects on.")
	projectsListMultipleCmd.Flags().BoolVar(&projectsListMultipleFlags.archived, "archived", false, "Only return projects whose `archived` field takes on the value of this parameter.")
	projectsListMultipleCmd.Flags().StringSliceVar(&projectsListMultipleFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	projectsCmd.AddCommand(projectsListMultipleCmd)
}

func runProjectsListMultiple(cmd *cobra.Command, args []string) error {
	...
	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/projects", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", projectsListMultipleFlags.optPretty)
	}
	if cmd.Flags().Changed("limit") {
		req.QueryParams["limit"] = fmt.Sprintf("%v", projectsListMultipleFlags.limit)
	}
	if cmd.Flags().Changed("offset") {
		req.QueryParams["offset"] = fmt.Sprintf("%v", projectsListMultipleFlags.offset)
	}
	if cmd.Flags().Changed("workspace") {
		req.QueryParams["workspace"] = fmt.Sprintf("%v", projectsListMultipleFlags.workspace)
	}
	if cmd.Flags().Changed("team") {
		req.QueryParams["team"] = fmt.Sprintf("%v", projectsListMultipleFlags.team)
	}
	if cmd.Flags().Changed("archived") {
		req.QueryParams["archived"] = fmt.Sprintf("%v", projectsListMultipleFlags.archived)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = projectsListMultipleFlags.optFields
	}
