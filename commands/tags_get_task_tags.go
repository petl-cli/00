...
var tagsGetTaskTagsCmd = &cobra.Command{
	Use:   "get-task-tags",
	Short: "Get a task's tags",
	RunE:  runTagsGetTaskTags,
}

var tagsGetTaskTagsFlags struct {
	taskGid   string
	optPretty bool
	limit     int
	offset    string
...
func init() {
	tagsGetTaskTagsCmd.Flags().StringVar(&tagsGetTaskTagsFlags.taskGid, "task-gid", "", "The task to operate on.")
	tagsGetTaskTagsCmd.MarkFlagRequired("task-gid")
	tagsGetTaskTagsCmd.Flags().BoolVar(&tagsGetTaskTagsFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	tagsGetTaskTagsCmd.Flags().IntVar(&tagsGetTaskTagsFlags.limit, "limit", 0, "Results per page. The number of objects to return per page. The value must be between 1 and 100.")
	tagsGetTaskTagsCmd.Flags().StringVar(&tagsGetTaskTagsFlags.offset, "offset", "", "Offset token. An offset to the next page returned by the API. A pagination request will return an offset token, which can be used as an input parameter to the next request. If an offset is not passed in, the API will return the first page of results. *Note: You can only pass in an offset that was returned to you via a previously paginated request.*")
	tagsGetTaskTagsCmd.Flags().StringSliceVar(&tagsGetTaskTagsFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	tagsCmd.AddCommand(tagsGetTaskTagsCmd)
}

func runTagsGetTaskTags(cmd *cobra.Command, args []string) error {
	...
	pathParams["task_gid"] = fmt.Sprintf("%v", tagsGetTaskTagsFlags.taskGid)

	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/tasks/{task_gid}/tags", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", tagsGetTaskTagsFlags.optPretty)
	}
	if cmd.Flags().Changed("limit") {
		req.QueryParams["limit"] = fmt.Sprintf("%v", tagsGetTaskTagsFlags.limit)
	}
	if cmd.Flags().Changed("offset") {
		req.QueryParams["offset"] = fmt.Sprintf("%v", tagsGetTaskTagsFlags.offset)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = tagsGetTaskTagsFlags.optFields
	}
