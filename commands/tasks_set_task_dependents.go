...
var tasksSetTaskDependentsCmd = &cobra.Command{
	Use:   "set-task-dependents",
	Short: "Set dependents for a task",
	RunE:  runTasksSetTaskDependents,
}

var tasksSetTaskDependentsFlags struct {
	taskGid   string
	optPretty bool
	body      string
}
...
func init() {
	tasksSetTaskDependentsCmd.Flags().StringVar(&tasksSetTaskDependentsFlags.taskGid, "task-gid", "", "The task to operate on.")
	tasksSetTaskDependentsCmd.MarkFlagRequired("task-gid")
	tasksSetTaskDependentsCmd.Flags().BoolVar(&tasksSetTaskDependentsFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	tasksSetTaskDependentsCmd.Flags().StringVar(&tasksSetTaskDependentsFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	tasksCmd.AddCommand(tasksSetTaskDependentsCmd)
}

func runTasksSetTaskDependents(cmd *cobra.Command, args []string) error {
	...
	pathParams["task_gid"] = fmt.Sprintf("%v", tasksSetTaskDependentsFlags.taskGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/tasks/{task_gid}/addDependents", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", tasksSetTaskDependentsFlags.optPretty)
	}
	...
	if tasksSetTaskDependentsFlags.body != "" {
		if err := json.Unmarshal([]byte(tasksSetTaskDependentsFlags.body), &bodyMap); err != nil {
