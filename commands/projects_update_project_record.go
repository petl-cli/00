...
var projectsUpdateProjectRecordCmd = &cobra.Command{
	Use:   "update-project-record",
	Short: "Update a project",
	RunE:  runProjectsUpdateProjectRecord,
}

var projectsUpdateProjectRecordFlags struct {
	projectGid string
	optPretty  bool
	optFields  []string
	data       string
...
func init() {
	projectsUpdateProjectRecordCmd.Flags().StringVar(&projectsUpdateProjectRecordFlags.projectGid, "project-gid", "", "Globally unique identifier for the project.")
	projectsUpdateProjectRecordCmd.MarkFlagRequired("project-gid")
	projectsUpdateProjectRecordCmd.Flags().BoolVar(&projectsUpdateProjectRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	projectsUpdateProjectRecordCmd.Flags().StringSliceVar(&projectsUpdateProjectRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	projectsUpdateProjectRecordCmd.Flags().StringVar(&projectsUpdateProjectRecordFlags.data, "data", "", "")
	// Note: body fields are not MarkFlagRequired — --body JSON satisfies them too.
	projectsUpdateProjectRecordCmd.Flags().StringVar(&projectsUpdateProjectRecordFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	projectsCmd.AddCommand(projectsUpdateProjectRecordCmd)
}

func runProjectsUpdateProjectRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["project_gid"] = fmt.Sprintf("%v", projectsUpdateProjectRecordFlags.projectGid)

	req := &httpclient.Request{
		Method:      "PUT",
		Path:        httpclient.SubstitutePath("/projects/{project_gid}", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", projectsUpdateProjectRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = projectsUpdateProjectRecordFlags.optFields
	}
	...
	if projectsUpdateProjectRecordFlags.body != "" {
		if err := json.Unmarshal([]byte(projectsUpdateProjectRecordFlags.body), &bodyMap); err != nil {
	...
	if cmd.Flags().Changed("data") {
		bodyMap["data"] = projectsUpdateProjectRecordFlags.data
	}
	req.Body = bodyMap
