...
var tasksAddProjectToTaskCmd = &cobra.Command{
	Use:   "add-project-to-task",
	Short: "Add a project to a task",
	RunE:  runTasksAddProjectToTask,
}

var tasksAddProjectToTaskFlags struct {
	taskGid   string
	optPretty bool
	body      string
}
...
func init() {
	tasksAddProjectToTaskCmd.Flags().StringVar(&tasksAddProjectToTaskFlags.taskGid, "task-gid", "", "The task to operate on.")
	tasksAddProjectToTaskCmd.MarkFlagRequired("task-gid")
	tasksAddProjectToTaskCmd.Flags().BoolVar(&tasksAddProjectToTaskFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	tasksAddProjectToTaskCmd.Flags().StringVar(&tasksAddProjectToTaskFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	tasksCmd.AddCommand(tasksAddProjectToTaskCmd)
}

func runTasksAddProjectToTask(cmd *cobra.Command, args []string) error {
	...
	pathParams["task_gid"] = fmt.Sprintf("%v", tasksAddProjectToTaskFlags.taskGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/tasks/{task_gid}/addProject", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", tasksAddProjectToTaskFlags.optPretty)
	}
	...
	if tasksAddProjectToTaskFlags.body != "" {
		if err := json.Unmarshal([]byte(tasksAddProjectToTaskFlags.body), &bodyMap); err != nil {
