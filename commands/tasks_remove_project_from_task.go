...
var tasksRemoveProjectFromTaskCmd = &cobra.Command{
	Use:   "remove-project-from-task",
	Short: "Remove a project from a task",
	RunE:  runTasksRemoveProjectFromTask,
}

var tasksRemoveProjectFromTaskFlags struct {
	taskGid   string
	optPretty bool
	body      string
}
...
func init() {
	tasksRemoveProjectFromTaskCmd.Flags().StringVar(&tasksRemoveProjectFromTaskFlags.taskGid, "task-gid", "", "The task to operate on.")
	tasksRemoveProjectFromTaskCmd.MarkFlagRequired("task-gid")
	tasksRemoveProjectFromTaskCmd.Flags().BoolVar(&tasksRemoveProjectFromTaskFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	tasksRemoveProjectFromTaskCmd.Flags().StringVar(&tasksRemoveProjectFromTaskFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	tasksCmd.AddCommand(tasksRemoveProjectFromTaskCmd)
}

func runTasksRemoveProjectFromTask(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "remove-project-from-task",
	...
	pathParams["task_gid"] = fmt.Sprintf("%v", tasksRemoveProjectFromTaskFlags.taskGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/tasks/{task_gid}/removeProject", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", tasksRemoveProjectFromTaskFlags.optPretty)
	}
	...
	if tasksRemoveProjectFromTaskFlags.body != "" {
		if err := json.Unmarshal([]byte(tasksRemoveProjectFromTaskFlags.body), &bodyMap); err != nil {
