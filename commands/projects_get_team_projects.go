...
var projectsGetTeamProjectsCmd = &cobra.Command{
	Use:   "get-team-projects",
	Short: "Get a team's projects",
	RunE:  runProjectsGetTeamProjects,
}

var projectsGetTeamProjectsFlags struct {
	teamGid   string
	optPretty bool
	limit     int
	offset    string
...
func init() {
	projectsGetTeamProjectsCmd.Flags().StringVar(&projectsGetTeamProjectsFlags.teamGid, "team-gid", "", "Globally unique identifier for the team.")
	projectsGetTeamProjectsCmd.MarkFlagRequired("team-gid")
	projectsGetTeamProjectsCmd.Flags().BoolVar(&projectsGetTeamProjectsFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	projectsGetTeamProjectsCmd.Flags().IntVar(&projectsGetTeamProjectsFlags.limit, "limit", 0, "Results per page. The number of objects to return per page. The value must be between 1 and 100.")
	projectsGetTeamProjectsCmd.Flags().StringVar(&projectsGetTeamProjectsFlags.offset, "offset", "", "Offset token. An offset to the next page returned by the API. A pagination request will return an offset token, which can be used as an input parameter to the next request. If an offset is not passed in, the API will return the first page of results. *Note: You can only pass in an offset that was returned to you via a previously paginated request.*")
	projectsGetTeamProjectsCmd.Flags().BoolVar(&projectsGetTeamProjectsFlags.archived, "archived", false, "Only return projects whose `archived` field takes on the value of this parameter.")
	projectsGetTeamProjectsCmd.Flags().StringSliceVar(&projectsGetTeamProjectsFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	projectsCmd.AddCommand(projectsGetTeamProjectsCmd)
}

func runProjectsGetTeamProjects(cmd *cobra.Command, args []string) error {
	...
	pathParams["team_gid"] = fmt.Sprintf("%v", projectsGetTeamProjectsFlags.teamGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", projectsGetTeamProjectsFlags.optPretty)
	}
	if cmd.Flags().Changed("limit") {
		req.QueryParams["limit"] = fmt.Sprintf("%v", projectsGetTeamProjectsFlags.limit)
	}
	if cmd.Flags().Changed("offset") {
		req.QueryParams["offset"] = fmt.Sprintf("%v", projectsGetTeamProjectsFlags.offset)
	}
	if cmd.Flags().Changed("archived") {
		req.QueryParams["archived"] = fmt.Sprintf("%v", projectsGetTeamProjectsFlags.archived)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = projectsGetTeamProjectsFlags.optFields
	}
