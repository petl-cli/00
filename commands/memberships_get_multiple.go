...
var membershipsGetMultipleCmd = &cobra.Command{
	Use:   "get-multiple",
	Short: "Get multiple memberships",
	RunE:  runMembershipsGetMultiple,
}

var membershipsGetMultipleFlags struct {
	optPretty bool
	parent    string
	member    string
	limit     int
...
func init() {
	membershipsGetMultipleCmd.Flags().BoolVar(&membershipsGetMultipleFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	membershipsGetMultipleCmd.Flags().StringVar(&membershipsGetMultipleFlags.parent, "parent", "", "Globally unique identifier for `goal` or `project`.")
	membershipsGetMultipleCmd.Flags().StringVar(&membershipsGetMultipleFlags.member, "member", "", "Globally unique identifier for `team` or `user`.")
	membershipsGetMultipleCmd.Flags().IntVar(&membershipsGetMultipleFlags.limit, "limit", 0, "Results per page. The number of objects to return per page. The value must be between 1 and 100.")
	membershipsGetMultipleCmd.Flags().StringVar(&membershipsGetMultipleFlags.offset, "offset", "", "Offset token. An offset to the next page returned by the API. A pagination request will return an offset token, which can be used as an input parameter to the next request. If an offset is not passed in, the API will return the first page of results. *Note: You can only pass in an offset that was returned to you via a previously paginated request.*")
	membershipsGetMultipleCmd.Flags().StringSliceVar(&membershipsGetMultipleFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	membershipsCmd.AddCommand(membershipsGetMultipleCmd)
}

func runMembershipsGetMultiple(cmd *cobra.Command, args []string) error {
	...
	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/memberships", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", membershipsGetMultipleFlags.optPretty)
	}
	if cmd.Flags().Changed("parent") {
		req.QueryParams["parent"] = fmt.Sprintf("%v", membershipsGetMultipleFlags.parent)
	}
	if cmd.Flags().Changed("member") {
		req.QueryParams["member"] = fmt.Sprintf("%v", membershipsGetMultipleFlags.member)
	}
	if cmd.Flags().Changed("limit") {
		req.QueryParams["limit"] = fmt.Sprintf("%v", membershipsGetMultipleFlags.limit)
	}
	if cmd.Flags().Changed("offset") {
		req.QueryParams["offset"] = fmt.Sprintf("%v", membershipsGetMultipleFlags.offset)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = membershipsGetMultipleFlags.optFields
	}
