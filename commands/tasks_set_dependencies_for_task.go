...
var tasksSetDependenciesForTaskCmd = &cobra.Command{
	Use:   "set-dependencies-for-task",
	Short: "Set dependencies for a task",
	RunE:  runTasksSetDependenciesForTask,
}

var tasksSetDependenciesForTaskFlags struct {
	taskGid   string
	optPretty bool
	body      string
}
...
func init() {
	tasksSetDependenciesForTaskCmd.Flags().StringVar(&tasksSetDependenciesForTaskFlags.taskGid, "task-gid", "", "The task to operate on.")
	tasksSetDependenciesForTaskCmd.MarkFlagRequired("task-gid")
	tasksSetDependenciesForTaskCmd.Flags().BoolVar(&tasksSetDependenciesForTaskFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	tasksSetDependenciesForTaskCmd.Flags().StringVar(&tasksSetDependenciesForTaskFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	tasksCmd.AddCommand(tasksSetDependenciesForTaskCmd)
}

func runTasksSetDependenciesForTask(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "set-dependencies-for-task",
	...
	pathParams["task_gid"] = fmt.Sprintf("%v", tasksSetDependenciesForTaskFlags.taskGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/tasks/{task_gid}/addDependencies", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", tasksSetDependenciesForTaskFlags.optPretty)
	}
	...
	if tasksSetDependenciesForTaskFlags.body != "" {
		if err := json.Unmarshal([]byte(tasksSetDependenciesForTaskFlags.body), &bodyMap); err != nil {
