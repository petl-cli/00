...
var goalsUpdateGoalRecordCmd = &cobra.Command{
	Use:   "update-goal-record",
	Short: "Update a goal",
	RunE:  runGoalsUpdateGoalRecord,
}

var goalsUpdateGoalRecordFlags struct {
	goalGid   string
	optPretty bool
	optFields []string
	data      string
...
func init() {
	goalsUpdateGoalRecordCmd.Flags().StringVar(&goalsUpdateGoalRecordFlags.goalGid, "goal-gid", "", "Globally unique identifier for the goal.")
	goalsUpdateGoalRecordCmd.MarkFlagRequired("goal-gid")
	goalsUpdateGoalRecordCmd.Flags().BoolVar(&goalsUpdateGoalRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	goalsUpdateGoalRecordCmd.Flags().StringSliceVar(&goalsUpdateGoalRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	goalsUpdateGoalRecordCmd.Flags().StringVar(&goalsUpdateGoalRecordFlags.data, "data", "", "")
	// Note: body fields are not MarkFlagRequired — --body JSON satisfies them too.
	goalsUpdateGoalRecordCmd.Flags().StringVar(&goalsUpdateGoalRecordFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	goalsCmd.AddCommand(goalsUpdateGoalRecordCmd)
}

func runGoalsUpdateGoalRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["goal_gid"] = fmt.Sprintf("%v", goalsUpdateGoalRecordFlags.goalGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", goalsUpdateGoalRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = goalsUpdateGoalRecordFlags.optFields
	}
	...
	if goalsUpdateGoalRecordFlags.body != "" {
		if err := json.Unmarshal([]byte(goalsUpdateGoalRecordFlags.body), &bodyMap); err != nil {
	...
	if cmd.Flags().Changed("data") {
		bodyMap["data"] = goalsUpdateGoalRecordFlags.data
	}
	req.Body = bodyMap
