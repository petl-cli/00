...
var tasksGetTaskRecordCmd = &cobra.Command{
	Use:   "get-task-record",
	Short: "Get a task",
	RunE:  runTasksGetTaskRecord,
}

var tasksGetTaskRecordFlags struct {
	taskGid   string
	optPretty bool
	optFields []string
}
...
func init() {
	tasksGetTaskRecordCmd.Flags().StringVar(&tasksGetTaskRecordFlags.taskGid, "task-gid", "", "The task to operate on.")
	tasksGetTaskRecordCmd.MarkFlagRequired("task-gid")
	tasksGetTaskRecordCmd.Flags().BoolVar(&tasksGetTaskRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	tasksGetTaskRecordCmd.Flags().StringSliceVar(&tasksGetTaskRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	tasksCmd.AddCommand(tasksGetTaskRecordCmd)
}

func runTasksGetTaskRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["task_gid"] = fmt.Sprintf("%v", tasksGetTaskRecordFlags.taskGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", tasksGetTaskRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = tasksGetTaskRecordFlags.optFields
	}
