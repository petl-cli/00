...
var allocationsGetRecordByIdCmd = &cobra.Command{
	Use:   "get-record-by-id",
	Short: "Get an allocation",
	RunE:  runAllocationsGetRecordById,
}

var allocationsGetRecordByIdFlags struct {
...
func init() {
	allocationsGetRecordByIdCmd.Flags().StringVar(&allocationsGetRecordByIdFlags.allocationGid, "allocation-gid", "", "Globally unique identifier for the allocation.")
	allocationsGetRecordByIdCmd.MarkFlagRequired("allocation-gid")
	allocationsGetRecordByIdCmd.Flags().BoolVar(&allocationsGetRecordByIdFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	allocationsGetRecordByIdCmd.Flags().StringSliceVar(&allocationsGetRecordByIdFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	allocationsCmd.AddCommand(allocationsGetRecordByIdCmd)
}

func runAllocationsGetRecordById(cmd *cobra.Command, args []string) error {
	...
	pathParams["allocation_gid"] = fmt.Sprintf("%v", allocationsGetRecordByIdFlags.allocationGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", allocationsGetRecordByIdFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = allocationsGetRecordByIdFlags.optFields
	}
