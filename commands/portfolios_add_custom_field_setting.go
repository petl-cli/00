...
var portfoliosAddCustomFieldSettingCmd = &cobra.Command{
	Use:   "add-custom-field-setting",
	Short: "Add a custom field to a portfolio",
	RunE:  runPortfoliosAddCustomFieldSetting,
}

var portfoliosAddCustomFieldSettingFlags struct {
	portfolioGid string
	optPretty    bool
	body         string
}
...
func init() {
	portfoliosAddCustomFieldSettingCmd.Flags().StringVar(&portfoliosAddCustomFieldSettingFlags.portfolioGid, "portfolio-gid", "", "Globally unique identifier for the portfolio.")
	portfoliosAddCustomFieldSettingCmd.MarkFlagRequired("portfolio-gid")
	portfoliosAddCustomFieldSettingCmd.Flags().BoolVar(&portfoliosAddCustomFieldSettingFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	portfoliosAddCustomFieldSettingCmd.Flags().StringVar(&portfoliosAddCustomFieldSettingFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	portfoliosCmd.AddCommand(portfoliosAddCustomFieldSettingCmd)
}

func runPortfoliosAddCustomFieldSetting(cmd *cobra.Command, args []string) error {
	...
	pathParams["portfolio_gid"] = fmt.Sprintf("%v", portfoliosAddCustomFieldSettingFlags.portfolioGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/portfolios/{portfolio_gid}/addCustomFieldSetting", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", portfoliosAddCustomFieldSettingFlags.optPretty)
	}
	...
	if portfoliosAddCustomFieldSettingFlags.body != "" {
		if err := json.Unmarshal([]byte(portfoliosAddCustomFieldSettingFlags.body), &bodyMap); err != nil {
