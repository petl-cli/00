...
var teamsUpdateTeamRecordCmd = &cobra.Command{
	Use:   "update-team-record",
	Short: "Update a team",
	RunE:  runTeamsUpdateTeamRecord,
}

var teamsUpdateTeamRecordFlags struct {
	teamGid   string
	optPretty bool
	optFields []string
	data      string
...
func init() {
	teamsUpdateTeamRecordCmd.Flags().StringVar(&teamsUpdateTeamRecordFlags.teamGid, "team-gid", "", "Globally unique identifier for the team.")
	teamsUpdateTeamRecordCmd.MarkFlagRequired("team-gid")
	teamsUpdateTeamRecordCmd.Flags().BoolVar(&teamsUpdateTeamRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	teamsUpdateTeamRecordCmd.Flags().StringSliceVar(&teamsUpdateTeamRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	teamsUpdateTeamRecordCmd.Flags().StringVar(&teamsUpdateTeamRecordFlags.data, "data", "", "")
	// Note: body fields are not MarkFlagRequired — --body JSON satisfies them too.
	teamsUpdateTeamRecordCmd.Flags().StringVar(&teamsUpdateTeamRecordFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	teamsCmd.AddCommand(teamsUpdateTeamRecordCmd)
}

func runTeamsUpdateTeamRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["team_gid"] = fmt.Sprintf("%v", teamsUpdateTeamRecordFlags.teamGid)

	req := &httpclient.Request{
		Method:      "PUT",
		Path:        httpclient.SubstitutePath("/teams/{team_gid}", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", teamsUpdateTeamRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = teamsUpdateTeamRecordFlags.optFields
	}
	...
	if teamsUpdateTeamRecordFlags.body != "" {
		if err := json.Unmarshal([]byte(teamsUpdateTeamRecordFlags.body), &bodyMap); err != nil {
	...
	if cmd.Flags().Changed("data") {
		bodyMap["data"] = teamsUpdateTeamRecordFlags.data
	}
	req.Body = bodyMap
