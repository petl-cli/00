...
var tasksGetAllDependenciesCmd = &cobra.Command{
	Use:   "get-all-dependencies",
	Short: "Get dependencies from a task",
	RunE:  runTasksGetAllDependencies,
}

var tasksGetAllDependenciesFlags struct {
	taskGid   string
	optPretty bool
	limit     int
	offset    string
...
func init() {
	tasksGetAllDependenciesCmd.Flags().StringVar(&tasksGetAllDependenciesFlags.taskGid, "task-gid", "", "The task to operate on.")
	tasksGetAllDependenciesCmd.MarkFlagRequired("task-gid")
	tasksGetAllDependenciesCmd.Flags().BoolVar(&tasksGetAllDependenciesFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	tasksGetAllDependenciesCmd.Flags().IntVar(&tasksGetAllDependenciesFlags.limit, "limit", 0, "Results per page. The number of objects to return per page. The value must be between 1 and 100.")
	tasksGetAllDependenciesCmd.Flags().StringVar(&tasksGetAllDependenciesFlags.offset, "offset", "", "Offset token. An offset to the next page returned by the API. A pagination request will return an offset token, which can be used as an input parameter to the next request. If an offset is not passed in, the API will return the first page of results. *Note: You can only pass in an offset that was returned to you via a previously paginated request.*")
	tasksGetAllDependenciesCmd.Flags().StringSliceVar(&tasksGetAllDependenciesFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	tasksCmd.AddCommand(tasksGetAllDependenciesCmd)
}

func runTasksGetAllDependencies(cmd *cobra.Command, args []string) error {
	...
	pathParams["task_gid"] = fmt.Sprintf("%v", tasksGetAllDependenciesFlags.taskGid)

	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/tasks/{task_gid}/dependencies", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", tasksGetAllDependenciesFlags.optPretty)
	}
	if cmd.Flags().Changed("limit") {
		req.QueryParams["limit"] = fmt.Sprintf("%v", tasksGetAllDependenciesFlags.limit)
	}
	if cmd.Flags().Changed("offset") {
		req.QueryParams["offset"] = fmt.Sprintf("%v", tasksGetAllDependenciesFlags.offset)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = tasksGetAllDependenciesFlags.optFields
	}
