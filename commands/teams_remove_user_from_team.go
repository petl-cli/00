...
var teamsRemoveUserFromTeamCmd = &cobra.Command{
	Use:   "remove-user-from-team",
	Short: "Remove a user from a team",
	RunE:  runTeamsRemoveUserFromTeam,
}

var teamsRemoveUserFromTeamFlags struct {
	teamGid   string
	optPretty bool
	body      string
}
...
func init() {
	teamsRemoveUserFromTeamCmd.Flags().StringVar(&teamsRemoveUserFromTeamFlags.teamGid, "team-gid", "", "Globally unique identifier for the team.")
	teamsRemoveUserFromTeamCmd.MarkFlagRequired("team-gid")
	teamsRemoveUserFromTeamCmd.Flags().BoolVar(&teamsRemoveUserFromTeamFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	teamsRemoveUserFromTeamCmd.Flags().StringVar(&teamsRemoveUserFromTeamFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	teamsCmd.AddCommand(teamsRemoveUserFromTeamCmd)
}

func runTeamsRemoveUserFromTeam(cmd *cobra.Command, args []string) error {
	...
	pathParams["team_gid"] = fmt.Sprintf("%v", teamsRemoveUserFromTeamFlags.teamGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/teams/{team_gid}/removeUser", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", teamsRemoveUserFromTeamFlags.optPretty)
	}
	...
	if teamsRemoveUserFromTeamFlags.body != "" {
		if err := json.Unmarshal([]byte(teamsRemoveUserFromTeamFlags.body), &bodyMap); err != nil {
