...
var projectBriefsRemoveBriefCmd = &cobra.Command{
	Use:   "remove-brief",
	Short: "Delete a project brief",
	RunE:  runProjectBriefsRemoveBrief,
}

var projectBriefsRemoveBriefFlags struct {
	projectBriefGid string
	optPretty       bool
}

func init() {
	projectBriefsRemoveBriefCmd.Flags().StringVar(&projectBriefsRemoveBriefFlags.projectBriefGid, "project-brief-gid", "", "Globally unique identifier for the project brief.")
	projectBriefsRemoveBriefCmd.MarkFlagRequired("project-brief-gid")
	projectBriefsRemoveBriefCmd.Flags().BoolVar(&projectBriefsRemoveBriefFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")

	projectBriefsCmd.AddCommand(projectBriefsRemoveBriefCmd)
}

func runProjectBriefsRemoveBrief(cmd *cobra.Command, args []string) error {
	...
	pathParams["project_brief_gid"] = fmt.Sprintf("%v", projectBriefsRemoveBriefFlags.projectBriefGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", projectBriefsRemoveBriefFlags.optPretty)
	}
