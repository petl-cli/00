...
var allocationsUpdateRecordByIdCmd = &cobra.Command{
	Use:   "update-record-by-id",
	Short: "Update an allocation",
	RunE:  runAllocationsUpdateRecordById,
}

var allocationsUpdateRecordByIdFlags struct {
...
func init() {
	allocationsUpdateRecordByIdCmd.Flags().StringVar(&allocationsUpdateRecordByIdFlags.allocationGid, "allocation-gid", "", "Globally unique identifier for the allocation.")
	allocationsUpdateRecordByIdCmd.MarkFlagRequired("allocation-gid")
	allocationsUpdateRecordByIdCmd.Flags().BoolVar(&allocationsUpdateRecordByIdFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	allocationsUpdateRecordByIdCmd.Flags().StringSliceVar(&allocationsUpdateRecordByIdFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	allocationsUpdateRecordByIdCmd.Flags().StringVar(&allocationsUpdateRecordByIdFlags.data, "data", "", "")
	// Note: body fields are not MarkFlagRequired — --body JSON satisfies them too.
	allocationsUpdateRecordByIdCmd.Flags().StringVar(&allocationsUpdateRecordByIdFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	allocationsCmd.AddCommand(allocationsUpdateRecordByIdCmd)
}

func runAllocationsUpdateRecordById(cmd *cobra.Command, args []string) error {
	...
	pathParams["allocation_gid"] = fmt.Sprintf("%v", allocationsUpdateRecordByIdFlags.allocationGid)

	req := &httpclient.Request{
		Method:      "PUT",
		Path:        httpclient.SubstitutePath("/allocations/{allocation_gid}", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", allocationsUpdateRecordByIdFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = allocationsUpdateRecordByIdFlags.optFields
	}
	...
	if allocationsUpdateRecordByIdFlags.body != "" {
		if err := json.Unmarshal([]byte(allocationsUpdateRecordByIdFlags.body), &bodyMap); err != nil {
	...
	if cmd.Flags().Changed("data") {
		bodyMap["data"] = allocationsUpdateRecordByIdFlags.data
	}
	req.Body = bodyMap
