...
var workspacesAddUserToWorkspaceCmd = &cobra.Command{
	Use:   "add-user-to-workspace",
	Short: "Add a user to a workspace or organization",
	RunE:  runWorkspacesAddUserToWorkspace,
}

var workspacesAddUserToWorkspaceFlags struct {
	workspaceGid string
	optPretty    bool
	optFields    []string
	body         string
...
func init() {
	workspacesAddUserToWorkspaceCmd.Flags().StringVar(&workspacesAddUserToWorkspaceFlags.workspaceGid, "workspace-gid", "", "Globally unique identifier for the workspace or organization.")
	workspacesAddUserToWorkspaceCmd.MarkFlagRequired("workspace-gid")
	workspacesAddUserToWorkspaceCmd.Flags().BoolVar(&workspacesAddUserToWorkspaceFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	workspacesAddUserToWorkspaceCmd.Flags().StringSliceVar(&workspacesAddUserToWorkspaceFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	workspacesAddUserToWorkspaceCmd.Flags().StringVar(&workspacesAddUserToWorkspaceFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	workspacesCmd.AddCommand(workspacesAddUserToWorkspaceCmd)
}

func runWorkspacesAddUserToWorkspace(cmd *cobra.Command, args []string) error {
	...
	pathParams["workspace_gid"] = fmt.Sprintf("%v", workspacesAddUserToWorkspaceFlags.workspaceGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/workspaces/{workspace_gid}/addUser", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", workspacesAddUserToWorkspaceFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = workspacesAddUserToWorkspaceFlags.optFields
	}
	...
	if workspacesAddUserToWorkspaceFlags.body != "" {
		if err := json.Unmarshal([]byte(workspacesAddUserToWorkspaceFlags.body), &bodyMap); err != nil {
