...
var tagsGetRecordCmd = &cobra.Command{
	Use:   "get-record",
	Short: "Get a tag",
	RunE:  runTagsGetRecord,
}

var tagsGetRecordFlags struct {
	tagGid    string
	optPretty bool
	optFields []string
}
...
func init() {
	tagsGetRecordCmd.Flags().StringVar(&tagsGetRecordFlags.tagGid, "tag-gid", "", "Globally unique identifier for the tag.")
	tagsGetRecordCmd.MarkFlagRequired("tag-gid")
	tagsGetRecordCmd.Flags().BoolVar(&tagsGetRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	tagsGetRecordCmd.Flags().StringSliceVar(&tagsGetRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	tagsCmd.AddCommand(tagsGetRecordCmd)
}

func runTagsGetRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["tag_gid"] = fmt.Sprintf("%v", tagsGetRecordFlags.tagGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", tagsGetRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = tagsGetRecordFlags.optFields
	}
