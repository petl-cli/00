...
var statusUpdatesDeleteSpecificStatusUpdateCmd = &cobra.Command{
	Use:   "delete-specific-status-update",
	Short: "Delete a status update",
	RunE:  runStatusUpdatesDeleteSpecificStatusUpdate,
}

var statusUpdatesDeleteSpecificStatusUpdateFlags struct {
	statusUpdateGid string
	optPretty       bool
}

func init() {
	statusUpdatesDeleteSpecificStatusUpdateCmd.Flags().StringVar(&statusUpdatesDeleteSpecificStatusUpdateFlags.statusUpdateGid, "status-update-gid", "", "The status update to get.")
	statusUpdatesDeleteSpecificStatusUpdateCmd.MarkFlagRequired("status-update-gid")
	statusUpdatesDeleteSpecificStatusUpdateCmd.Flags().BoolVar(&statusUpdatesDeleteSpecificStatusUpdateFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")

	statusUpdatesCmd.AddCommand(statusUpdatesDeleteSpecificStatusUpdateCmd)
}

func runStatusUpdatesDeleteSpecificStatusUpdate(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "delete-specific-status-update",
	...
	pathParams["status_update_gid"] = fmt.Sprintf("%v", statusUpdatesDeleteSpecificStatusUpdateFlags.statusUpdateGid)

	req := &httpclient.Request{
		Method:      "DELETE",
		Path:        httpclient.SubstitutePath("/status_updates/{status_update_gid}", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", statusUpdatesDeleteSpecificStatusUpdateFlags.optPretty)
	}
