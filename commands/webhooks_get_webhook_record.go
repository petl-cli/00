...
var webhooksGetWebhookRecordCmd = &cobra.Command{
	Use:   "get-webhook-record",
	Short: "Get a webhook",
	RunE:  runWebhooksGetWebhookRecord,
}

var webhooksGetWebhookRecordFlags struct {
	webhookGid string
	optPretty  bool
	optFields  []string
}
...
func init() {
	webhooksGetWebhookRecordCmd.Flags().StringVar(&webhooksGetWebhookRecordFlags.webhookGid, "webhook-gid", "", "Globally unique identifier for the webhook.")
	webhooksGetWebhookRecordCmd.MarkFlagRequired("webhook-gid")
	webhooksGetWebhookRecordCmd.Flags().BoolVar(&webhooksGetWebhookRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	webhooksGetWebhookRecordCmd.Flags().StringSliceVar(&webhooksGetWebhookRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	webhooksCmd.AddCommand(webhooksGetWebhookRecordCmd)
}

func runWebhooksGetWebhookRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["webhook_gid"] = fmt.Sprintf("%v", webhooksGetWebhookRecordFlags.webhookGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", webhooksGetWebhookRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = webhooksGetWebhookRecordFlags.optFields
	}
