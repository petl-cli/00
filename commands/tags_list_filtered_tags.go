...
var tagsListFilteredTagsCmd = &cobra.Command{
	Use:   "list-filtered-tags",
	Short: "Get multiple tags",
	RunE:  runTagsListFilteredTags,
}

var tagsListFilteredTagsFlags struct {
	optPretty bool
	limit     int
	offset    string
	workspace string
...
func init() {
	tagsListFilteredTagsCmd.Flags().BoolVar(&tagsListFilteredTagsFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	tagsListFilteredTagsCmd.Flags().IntVar(&tagsListFilteredTagsFlags.limit, "limit", 0, "Results per page. The number of objects to return per page. The value must be between 1 and 100.")
	tagsListFilteredTagsCmd.Flags().StringVar(&tagsListFilteredTagsFlags.offset, "offset", "", "Offset token. An offset to the next page returned by the API. A pagination request will return an offset token, which can be used as an input parameter to the next request. If an offset is not passed in, the API will return the first page of results. *Note: You can only pass in an offset that was returned to you via a previously paginated request.*")
	tagsListFilteredTagsCmd.Flags().StringVar(&tagsListFilteredTagsFlags.workspace, "workspace", "", "The workspace to filter tags on.")
	tagsListFilteredTagsCmd.Flags().StringSliceVar(&tagsListFilteredTagsFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	tagsCmd.AddCommand(tagsListFilteredTagsCmd)
}

func runTagsListFilteredTags(cmd *cobra.Command, args []string) error {
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", tagsListFilteredTagsFlags.optPretty)
	}
	if cmd.Flags().Changed("limit") {
		req.QueryParams["limit"] = fmt.Sprintf("%v", tagsListFilteredTagsFlags.limit)
	}
	if cmd.Flags().Changed("offset") {
		req.QueryParams["offset"] = fmt.Sprintf("%v", tagsListFilteredTagsFlags.offset)
	}
	if cmd.Flags().Changed("workspace") {
		req.QueryParams["workspace"] = fmt.Sprintf("%v", tagsListFilteredTagsFlags.workspace)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = tagsListFilteredTagsFlags.optFields
	}
