...
var projectsDeleteProjectByIdCmd = &cobra.Command{
	Use:   "delete-project-by-id",
	Short: "Delete a project",
	RunE:  runProjectsDeleteProjectById,
}

var projectsDeleteProjectByIdFlags struct {
	projectGid string
	optPretty  bool
}

func init() {
	projectsDeleteProjectByIdCmd.Flags().StringVar(&projectsDeleteProjectByIdFlags.projectGid, "project-gid", "", "Globally unique identifier for the project.")
	projectsDeleteProjectByIdCmd.MarkFlagRequired("project-gid")
	projectsDeleteProjectByIdCmd.Flags().BoolVar(&projectsDeleteProjectByIdFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")

	projectsCmd.AddCommand(projectsDeleteProjectByIdCmd)
}

func runProjectsDeleteProjectById(cmd *cobra.Command, args []string) error {
	...
	pathParams["project_gid"] = fmt.Sprintf("%v", projectsDeleteProjectByIdFlags.projectGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", projectsDeleteProjectByIdFlags.optPretty)
	}
