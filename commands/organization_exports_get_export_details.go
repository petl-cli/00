...
var organizationExportsGetExportDetailsCmd = &cobra.Command{
	Use:   "get-export-details",
	Short: "Get details on an org export request",
	RunE:  runOrganizationExportsGetExportDetails,
}

var organizationExportsGetExportDetailsFlags struct {
...
func init() {
	organizationExportsGetExportDetailsCmd.Flags().StringVar(&organizationExportsGetExportDetailsFlags.organizationExportGid, "organization-export-gid", "", "Globally unique identifier for the organization export.")
	organizationExportsGetExportDetailsCmd.MarkFlagRequired("organization-export-gid")
	organizationExportsGetExportDetailsCmd.Flags().BoolVar(&organizationExportsGetExportDetailsFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	organizationExportsGetExportDetailsCmd.Flags().StringSliceVar(&organizationExportsGetExportDetailsFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	organizationExportsCmd.AddCommand(organizationExportsGetExportDetailsCmd)
}

func runOrganizationExportsGetExportDetails(cmd *cobra.Command, args []string) error {
	...
	pathParams["organization_export_gid"] = fmt.Sprintf("%v", organizationExportsGetExportDetailsFlags.organizationExportGid)

	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/organization_exports/{organization_export_gid}", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", organizationExportsGetExportDetailsFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = organizationExportsGetExportDetailsFlags.optFields
	}
