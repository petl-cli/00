...
var userTaskListsGetRecordCmd = &cobra.Command{
	Use:   "get-record",
	Short: "Get a user task list",
	RunE:  runUserTaskListsGetRecord,
}

var userTaskListsGetRecordFlags struct {
...
func init() {
	userTaskListsGetRecordCmd.Flags().StringVar(&userTaskListsGetRecordFlags.userTaskListGid, "user-task-list-gid", "", "Globally unique identifier for the user task list.")
	userTaskListsGetRecordCmd.MarkFlagRequired("user-task-list-gid")
	userTaskListsGetRecordCmd.Flags().BoolVar(&userTaskListsGetRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	userTaskListsGetRecordCmd.Flags().StringSliceVar(&userTaskListsGetRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	userTaskListsCmd.AddCommand(userTaskListsGetRecordCmd)
}

func runUserTaskListsGetRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["user_task_list_gid"] = fmt.Sprintf("%v", userTaskListsGetRecordFlags.userTaskListGid)

	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/user_task_lists/{user_task_list_gid}", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", userTaskListsGetRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = userTaskListsGetRecordFlags.optFields
	}
