...
var attachmentsGetAttachmentRecordCmd = &cobra.Command{
	Use:   "get-attachment-record",
	Short: "Get an attachment",
	RunE:  runAttachmentsGetAttachmentRecord,
}

var attachmentsGetAttachmentRecordFlags struct {
...
func init() {
	attachmentsGetAttachmentRecordCmd.Flags().StringVar(&attachmentsGetAttachmentRecordFlags.attachmentGid, "attachment-gid", "", "Globally unique identifier for the attachment.")
	attachmentsGetAttachmentRecordCmd.MarkFlagRequired("attachment-gid")
	attachmentsGetAttachmentRecordCmd.Flags().BoolVar(&attachmentsGetAttachmentRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	attachmentsGetAttachmentRecordCmd.Flags().StringSliceVar(&attachmentsGetAttachmentRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	attachmentsCmd.AddCommand(attachmentsGetAttachmentRecordCmd)
}

func runAttachmentsGetAttachmentRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["attachment_gid"] = fmt.Sprintf("%v", attachmentsGetAttachmentRecordFlags.attachmentGid)

	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/attachments/{attachment_gid}", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", attachmentsGetAttachmentRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = attachmentsGetAttachmentRecordFlags.optFields
	}
