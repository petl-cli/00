...
var customFieldsUpdateFieldRecordCmd = &cobra.Command{
	Use:   "update-field-record",
	Short: "Update a custom field",
	RunE:  runCustomFieldsUpdateFieldRecord,
}

var customFieldsUpdateFieldRecordFlags struct {
...
func init() {
	customFieldsUpdateFieldRecordCmd.Flags().StringVar(&customFieldsUpdateFieldRecordFlags.customFieldGid, "custom-field-gid", "", "Globally unique identifier for the custom field.")
	customFieldsUpdateFieldRecordCmd.MarkFlagRequired("custom-field-gid")
	customFieldsUpdateFieldRecordCmd.Flags().BoolVar(&customFieldsUpdateFieldRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	customFieldsUpdateFieldRecordCmd.Flags().StringSliceVar(&customFieldsUpdateFieldRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	customFieldsUpdateFieldRecordCmd.Flags().StringVar(&customFieldsUpdateFieldRecordFlags.data, "data", "", "")
	// Note: body fields are not MarkFlagRequired — --body JSON satisfies them too.
	customFieldsUpdateFieldRecordCmd.Flags().StringVar(&customFieldsUpdateFieldRecordFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	customFieldsCmd.AddCommand(customFieldsUpdateFieldRecordCmd)
}

func runCustomFieldsUpdateFieldRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["custom_field_gid"] = fmt.Sprintf("%v", customFieldsUpdateFieldRecordFlags.customFieldGid)

	req := &httpclient.Request{
		Method:      "PUT",
		Path:        httpclient.SubstitutePath("/custom_fields/{custom_field_gid}", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", customFieldsUpdateFieldRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = customFieldsUpdateFieldRecordFlags.optFields
	}
	...
	if customFieldsUpdateFieldRecordFlags.body != "" {
		if err := json.Unmarshal([]byte(customFieldsUpdateFieldRecordFlags.body), &bodyMap); err != nil {
	...
	if cmd.Flags().Changed("data") {
		bodyMap["data"] = customFieldsUpdateFieldRecordFlags.data
	}
	req.Body = bodyMap
