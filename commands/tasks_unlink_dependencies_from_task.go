...
var tasksUnlinkDependenciesFromTaskCmd = &cobra.Command{
	Use:   "unlink-dependencies-from-task",
	Short: "Unlink dependencies from a task",
	RunE:  runTasksUnlinkDependenciesFromTask,
}

var tasksUnlinkDependenciesFromTaskFlags struct {
	taskGid   string
	optPretty bool
	body      string
}
...
func init() {
	tasksUnlinkDependenciesFromTaskCmd.Flags().StringVar(&tasksUnlinkDependenciesFromTaskFlags.taskGid, "task-gid", "", "The task to operate on.")
	tasksUnlinkDependenciesFromTaskCmd.MarkFlagRequired("task-gid")
	tasksUnlinkDependenciesFromTaskCmd.Flags().BoolVar(&tasksUnlinkDependenciesFromTaskFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	tasksUnlinkDependenciesFromTaskCmd.Flags().StringVar(&tasksUnlinkDependenciesFromTaskFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	tasksCmd.AddCommand(tasksUnlinkDependenciesFromTaskCmd)
}

func runTasksUnlinkDependenciesFromTask(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "unlink-dependencies-from-task",
	...
	pathParams["task_gid"] = fmt.Sprintf("%v", tasksUnlinkDependenciesFromTaskFlags.taskGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/tasks/{task_gid}/removeDependencies", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", tasksUnlinkDependenciesFromTaskFlags.optPretty)
	}
	...
	if tasksUnlinkDependenciesFromTaskFlags.body != "" {
		if err := json.Unmarshal([]byte(tasksUnlinkDependenciesFromTaskFlags.body), &bodyMap); err != nil {
