...
var portfoliosGetRecordCmd = &cobra.Command{
	Use:   "get-record",
	Short: "Get a portfolio",
	RunE:  runPortfoliosGetRecord,
}

var portfoliosGetRecordFlags struct {
	portfolioGid string
	optPretty    bool
	optFields    []string
}
...
func init() {
	portfoliosGetRecordCmd.Flags().StringVar(&portfoliosGetRecordFlags.portfolioGid, "portfolio-gid", "", "Globally unique identifier for the portfolio.")
	portfoliosGetRecordCmd.MarkFlagRequired("portfolio-gid")
	portfoliosGetRecordCmd.Flags().BoolVar(&portfoliosGetRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	portfoliosGetRecordCmd.Flags().StringSliceVar(&portfoliosGetRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	portfoliosCmd.AddCommand(portfoliosGetRecordCmd)
}

func runPortfoliosGetRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["portfolio_gid"] = fmt.Sprintf("%v", portfoliosGetRecordFlags.portfolioGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", portfoliosGetRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = portfoliosGetRecordFlags.optFields
	}
