...
var usersListTeamUsersCmd = &cobra.Command{
	Use:   "list-team-users",
	Short: "Get users in a team",
	RunE:  runUsersListTeamUsers,
}

var usersListTeamUsersFlags struct {
	teamGid   string
	optPretty bool
	offset    string
	optFields []string
...
func init() {
	usersListTeamUsersCmd.Flags().StringVar(&usersListTeamUsersFlags.teamGid, "team-gid", "", "Globally unique identifier for the team.")
	usersListTeamUsersCmd.MarkFlagRequired("team-gid")
	usersListTeamUsersCmd.Flags().BoolVar(&usersListTeamUsersFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	usersListTeamUsersCmd.Flags().StringVar(&usersListTeamUsersFlags.offset, "offset", "", "Offset token. An offset to the next page returned by the API. A pagination request will return an offset token, which can be used as an input parameter to the next request. If an offset is not passed in, the API will return the first page of results. *Note: You can only pass in an offset that was returned to you via a previously paginated request.*")
	usersListTeamUsersCmd.Flags().StringSliceVar(&usersListTeamUsersFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	usersCmd.AddCommand(usersListTeamUsersCmd)
}

func runUsersListTeamUsers(cmd *cobra.Command, args []string) error {
	...
	pathParams["team_gid"] = fmt.Sprintf("%v", usersListTeamUsersFlags.teamGid)

	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/teams/{team_gid}/users", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", usersListTeamUsersFlags.optPretty)
	}
	if cmd.Flags().Changed("offset") {
		req.QueryParams["offset"] = fmt.Sprintf("%v", usersListTeamUsersFlags.offset)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = usersListTeamUsersFlags.optFields
	}
