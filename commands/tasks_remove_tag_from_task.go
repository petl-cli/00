...
var tasksRemoveTagFromTaskCmd = &cobra.Command{
	Use:   "remove-tag-from-task",
	Short: "Remove a tag from a task",
	RunE:  runTasksRemoveTagFromTask,
}

var tasksRemoveTagFromTaskFlags struct {
	taskGid   string
	optPretty bool
	body      string
}
...
func init() {
	tasksRemoveTagFromTaskCmd.Flags().StringVar(&tasksRemoveTagFromTaskFlags.taskGid, "task-gid", "", "The task to operate on.")
	tasksRemoveTagFromTaskCmd.MarkFlagRequired("task-gid")
	tasksRemoveTagFromTaskCmd.Flags().BoolVar(&tasksRemoveTagFromTaskFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	tasksRemoveTagFromTaskCmd.Flags().StringVar(&tasksRemoveTagFromTaskFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	tasksCmd.AddCommand(tasksRemoveTagFromTaskCmd)
}

func runTasksRemoveTagFromTask(cmd *cobra.Command, args []string) error {
	...
	pathParams["task_gid"] = fmt.Sprintf("%v", tasksRemoveTagFromTaskFlags.taskGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/tasks/{task_gid}/removeTag", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", tasksRemoveTagFromTaskFlags.optPretty)
	}
	...
	if tasksRemoveTagFromTaskFlags.body != "" {
		if err := json.Unmarshal([]byte(tasksRemoveTagFromTaskFlags.body), &bodyMap); err != nil {
