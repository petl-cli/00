...
var projectsGetTaskCountsCmd = &cobra.Command{
	Use:   "get-task-counts",
	Short: "Get task count of a project",
	RunE:  runProjectsGetTaskCounts,
}

var projectsGetTaskCountsFlags struct {
	projectGid string
	optPretty  bool
	optFields  []string
}
...
func init() {
	projectsGetTaskCountsCmd.Flags().StringVar(&projectsGetTaskCountsFlags.projectGid, "project-gid", "", "Globally unique identifier for the project.")
	projectsGetTaskCountsCmd.MarkFlagRequired("project-gid")
	projectsGetTaskCountsCmd.Flags().BoolVar(&projectsGetTaskCountsFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	projectsGetTaskCountsCmd.Flags().StringSliceVar(&projectsGetTaskCountsFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	projectsCmd.AddCommand(projectsGetTaskCountsCmd)
}

func runProjectsGetTaskCounts(cmd *cobra.Command, args []string) error {
	...
	pathParams["project_gid"] = fmt.Sprintf("%v", projectsGetTaskCountsFlags.projectGid)

	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/projects/{project_gid}/task_counts", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", projectsGetTaskCountsFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = projectsGetTaskCountsFlags.optFields
	}
