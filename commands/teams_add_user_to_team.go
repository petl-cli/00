...
var teamsAddUserToTeamCmd = &cobra.Command{
	Use:   "add-user-to-team",
	Short: "Add a user to a team",
	RunE:  runTeamsAddUserToTeam,
}

var teamsAddUserToTeamFlags struct {
	teamGid   string
	optPretty bool
	optFields []string
	body      string
...
func init() {
	teamsAddUserToTeamCmd.Flags().StringVar(&teamsAddUserToTeamFlags.teamGid, "team-gid", "", "Globally unique identifier for the team.")
	teamsAddUserToTeamCmd.MarkFlagRequired("team-gid")
	teamsAddUserToTeamCmd.Flags().BoolVar(&teamsAddUserToTeamFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	teamsAddUserToTeamCmd.Flags().StringSliceVar(&teamsAddUserToTeamFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	teamsAddUserToTeamCmd.Flags().StringVar(&teamsAddUserToTeamFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	teamsCmd.AddCommand(teamsAddUserToTeamCmd)
}

func runTeamsAddUserToTeam(cmd *cobra.Command, args []string) error {
	...
	pathParams["team_gid"] = fmt.Sprintf("%v", teamsAddUserToTeamFlags.teamGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/teams/{team_gid}/addUser", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", teamsAddUserToTeamFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = teamsAddUserToTeamFlags.optFields
	}
	...
	if teamsAddUserToTeamFlags.body != "" {
		if err := json.Unmarshal([]byte(teamsAddUserToTeamFlags.body), &bodyMap); err != nil {
