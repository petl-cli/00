...
var tasksAddTagToTaskCmd = &cobra.Command{
	Use:   "add-tag-to-task",
	Short: "Add a tag to a task",
	RunE:  runTasksAddTagToTask,
}

var tasksAddTagToTaskFlags struct {
	taskGid   string
	optPretty bool
	body      string
}
...
func init() {
	tasksAddTagToTaskCmd.Flags().StringVar(&tasksAddTagToTaskFlags.taskGid, "task-gid", "", "The task to operate on.")
	tasksAddTagToTaskCmd.MarkFlagRequired("task-gid")
	tasksAddTagToTaskCmd.Flags().BoolVar(&tasksAddTagToTaskFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	tasksAddTagToTaskCmd.Flags().StringVar(&tasksAddTagToTaskFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	tasksCmd.AddCommand(tasksAddTagToTaskCmd)
}

func runTasksAddTagToTask(cmd *cobra.Command, args []string) error {
	...
	pathParams["task_gid"] = fmt.Sprintf("%v", tasksAddTagToTaskFlags.taskGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/tasks/{task_gid}/addTag", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", tasksAddTagToTaskFlags.optPretty)
	}
	...
	if tasksAddTagToTaskFlags.body != "" {
		if err := json.Unmarshal([]byte(tasksAddTagToTaskFlags.body), &bodyMap); err != nil {
