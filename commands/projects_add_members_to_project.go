...
var projectsAddMembersToProjectCmd = &cobra.Command{
	Use:   "add-members-to-project",
	Short: "Add users to a project",
	RunE:  runProjectsAddMembersToProject,
}

var projectsAddMembersToProjectFlags struct {
	projectGid string
	optPretty  bool
	optFields  []string
	body       string
...
func init() {
	projectsAddMembersToProjectCmd.Flags().StringVar(&projectsAddMembersToProjectFlags.projectGid, "project-gid", "", "Globally unique identifier for the project.")
	projectsAddMembersToProjectCmd.MarkFlagRequired("project-gid")
	projectsAddMembersToProjectCmd.Flags().BoolVar(&projectsAddMembersToProjectFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	projectsAddMembersToProjectCmd.Flags().StringSliceVar(&projectsAddMembersToProjectFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	projectsAddMembersToProjectCmd.Flags().StringVar(&projectsAddMembersToProjectFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	projectsCmd.AddCommand(projectsAddMembersToProjectCmd)
}

func runProjectsAddMembersToProject(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "add-members-to-project",
	...
	pathParams["project_gid"] = fmt.Sprintf("%v", projectsAddMembersToProjectFlags.projectGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/projects/{project_gid}/addMembers", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", projectsAddMembersToProjectFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = projectsAddMembersToProjectFlags.optFields
	}
	...
	if projectsAddMembersToProjectFlags.body != "" {
		if err := json.Unmarshal([]byte(projectsAddMembersToProjectFlags.body), &bodyMap); err != nil {
