...
var storiesDeleteStoryRecordCmd = &cobra.Command{
	Use:   "delete-story-record",
	Short: "Delete a story",
	RunE:  runStoriesDeleteStoryRecord,
}

var storiesDeleteStoryRecordFlags struct {
	storyGid  string
	optPretty bool
}

func init() {
	storiesDeleteStoryRecordCmd.Flags().StringVar(&storiesDeleteStoryRecordFlags.storyGid, "story-gid", "", "Globally unique identifier for the story.")
	storiesDeleteStoryRecordCmd.MarkFlagRequired("story-gid")
	storiesDeleteStoryRecordCmd.Flags().BoolVar(&storiesDeleteStoryRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")

	storiesCmd.AddCommand(storiesDeleteStoryRecordCmd)
}

func runStoriesDeleteStoryRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["story_gid"] = fmt.Sprintf("%v", storiesDeleteStoryRecordFlags.storyGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", storiesDeleteStoryRecordFlags.optPretty)
	}
