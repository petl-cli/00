...
var attachmentsGetAllForObjectCmd = &cobra.Command{
	Use:   "get-all-for-object",
	Short: "Get attachments from an object",
	RunE:  runAttachmentsGetAllForObject,
}

var attachmentsGetAllForObjectFlags struct {
	optPretty bool
	limit     int
	offset    string
	parent    string
...
func init() {
	attachmentsGetAllForObjectCmd.Flags().BoolVar(&attachmentsGetAllForObjectFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	attachmentsGetAllForObjectCmd.Flags().IntVar(&attachmentsGetAllForObjectFlags.limit, "limit", 0, "Results per page. The number of objects to return per page. The value must be between 1 and 100.")
	attachmentsGetAllForObjectCmd.Flags().StringVar(&attachmentsGetAllForObjectFlags.offset, "offset", "", "Offset token. An offset to the next page returned by the API. A pagination request will return an offset token, which can be used as an input parameter to the next request. If an offset is not passed in, the API will return the first page of results. *Note: You can only pass in an offset that was returned to you via a previously paginated request.*")
	attachmentsGetAllForObjectCmd.Flags().StringVar(&attachmentsGetAllForObjectFlags.parent, "parent", "", "Globally unique identifier for object to fetch statuses from. Must be a GID for a `project`, `project_brief`, or `task`.")
	attachmentsGetAllForObjectCmd.MarkFlagRequired("parent")
	attachmentsGetAllForObjectCmd.Flags().StringSliceVar(&attachmentsGetAllForObjectFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	attachmentsCmd.AddCommand(attachmentsGetAllForObjectCmd)
}

func runAttachmentsGetAllForObject(cmd *cobra.Command, args []string) error {
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", attachmentsGetAllForObjectFlags.optPretty)
	}
	if cmd.Flags().Changed("limit") {
		req.QueryParams["limit"] = fmt.Sprintf("%v", attachmentsGetAllForObjectFlags.limit)
	}
	if cmd.Flags().Changed("offset") {
		req.QueryParams["offset"] = fmt.Sprintf("%v", attachmentsGetAllForObjectFlags.offset)
	}
	if cmd.Flags().Changed("parent") {
		req.QueryParams["parent"] = fmt.Sprintf("%v", attachmentsGetAllForObjectFlags.parent)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = attachmentsGetAllForObjectFlags.optFields
	}
