...
var goalsUpdateMetricCurrentValueCmd = &cobra.Command{
	Use:   "update-metric-current-value",
	Short: "Update a goal metric",
	RunE:  runGoalsUpdateMetricCurrentValue,
}

var goalsUpdateMetricCurrentValueFlags struct {
	goalGid   string
	optPretty bool
	optFields []string
	data      string
...
func init() {
	goalsUpdateMetricCurrentValueCmd.Flags().StringVar(&goalsUpdateMetricCurrentValueFlags.goalGid, "goal-gid", "", "Globally unique identifier for the goal.")
	goalsUpdateMetricCurrentValueCmd.MarkFlagRequired("goal-gid")
	goalsUpdateMetricCurrentValueCmd.Flags().BoolVar(&goalsUpdateMetricCurrentValueFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	goalsUpdateMetricCurrentValueCmd.Flags().StringSliceVar(&goalsUpdateMetricCurrentValueFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	goalsUpdateMetricCurrentValueCmd.Flags().StringVar(&goalsUpdateMetricCurrentValueFlags.data, "data", "", "")
	// Note: body fields are not MarkFlagRequired — --body JSON satisfies them too.
	goalsUpdateMetricCurrentValueCmd.Flags().StringVar(&goalsUpdateMetricCurrentValueFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	goalsCmd.AddCommand(goalsUpdateMetricCurrentValueCmd)
}

func runGoalsUpdateMetricCurrentValue(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "update-metric-current-value",
	...
	pathParams["goal_gid"] = fmt.Sprintf("%v", goalsUpdateMetricCurrentValueFlags.goalGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/goals/{goal_gid}/setMetricCurrentValue", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", goalsUpdateMetricCurrentValueFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = goalsUpdateMetricCurrentValueFlags.optFields
	}
	...
	if goalsUpdateMetricCurrentValueFlags.body != "" {
		if err := json.Unmarshal([]byte(goalsUpdateMetricCurrentValueFlags.body), &bodyMap); err != nil {
	...
	if cmd.Flags().Changed("data") {
		bodyMap["data"] = goalsUpdateMetricCurrentValueFlags.data
	}
	req.Body = bodyMap
