...
var projectBriefsCreateNewRecordCmd = &cobra.Command{
	Use:   "create-new-record",
	Short: "Create a project brief",
	RunE:  runProjectBriefsCreateNewRecord,
}

var projectBriefsCreateNewRecordFlags struct {
	projectGid string
	optPretty  bool
	optFields  []string
	data       string
...
func init() {
	projectBriefsCreateNewRecordCmd.Flags().StringVar(&projectBriefsCreateNewRecordFlags.projectGid, "project-gid", "", "Globally unique identifier for the project.")
	projectBriefsCreateNewRecordCmd.MarkFlagRequired("project-gid")
	projectBriefsCreateNewRecordCmd.Flags().BoolVar(&projectBriefsCreateNewRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	projectBriefsCreateNewRecordCmd.Flags().StringSliceVar(&projectBriefsCreateNewRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	projectBriefsCreateNewRecordCmd.Flags().StringVar(&projectBriefsCreateNewRecordFlags.data, "data", "", "")
	// Note: body fields are not MarkFlagRequired — --body JSON satisfies them too.
	projectBriefsCreateNewRecordCmd.Flags().StringVar(&projectBriefsCreateNewRecordFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	projectBriefsCmd.AddCommand(projectBriefsCreateNewRecordCmd)
}

func runProjectBriefsCreateNewRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["project_gid"] = fmt.Sprintf("%v", projectBriefsCreateNewRecordFlags.projectGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/projects/{project_gid}/project_briefs", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", projectBriefsCreateNewRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = projectBriefsCreateNewRecordFlags.optFields
	}
	...
	if projectBriefsCreateNewRecordFlags.body != "" {
		if err := json.Unmarshal([]byte(projectBriefsCreateNewRecordFlags.body), &bodyMap); err != nil {
	...
	if cmd.Flags().Changed("data") {
		bodyMap["data"] = projectBriefsCreateNewRecordFlags.data
	}
	req.Body = bodyMap
