...
var portfoliosDeleteRecordCmd = &cobra.Command{
	Use:   "delete-record",
	Short: "Delete a portfolio",
	RunE:  runPortfoliosDeleteRecord,
}

var portfoliosDeleteRecordFlags struct {
	portfolioGid string
	optPretty    bool
}

func init() {
	portfoliosDeleteRecordCmd.Flags().StringVar(&portfoliosDeleteRecordFlags.portfolioGid, "portfolio-gid", "", "Globally unique identifier for the portfolio.")
	portfoliosDeleteRecordCmd.MarkFlagRequired("portfolio-gid")
	portfoliosDeleteRecordCmd.Flags().BoolVar(&portfoliosDeleteRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")

	portfoliosCmd.AddCommand(portfoliosDeleteRecordCmd)
}

func runPortfoliosDeleteRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["portfolio_gid"] = fmt.Sprintf("%v", portfoliosDeleteRecordFlags.portfolioGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", portfoliosDeleteRecordFlags.optPretty)
	}
