...
var timePeriodsGetFullRecordCmd = &cobra.Command{
	Use:   "get-full-record",
	Short: "Get a time period",
	RunE:  runTimePeriodsGetFullRecord,
}

var timePeriodsGetFullRecordFlags struct {
...
func init() {
	timePeriodsGetFullRecordCmd.Flags().StringVar(&timePeriodsGetFullRecordFlags.timePeriodGid, "time-period-gid", "", "Globally unique identifier for the time period.")
	timePeriodsGetFullRecordCmd.MarkFlagRequired("time-period-gid")
	timePeriodsGetFullRecordCmd.Flags().BoolVar(&timePeriodsGetFullRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	timePeriodsGetFullRecordCmd.Flags().StringSliceVar(&timePeriodsGetFullRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	timePeriodsCmd.AddCommand(timePeriodsGetFullRecordCmd)
}

func runTimePeriodsGetFullRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["time_period_gid"] = fmt.Sprintf("%v", timePeriodsGetFullRecordFlags.timePeriodGid)

	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/time_periods/{time_period_gid}", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", timePeriodsGetFullRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = timePeriodsGetFullRecordFlags.optFields
	}
