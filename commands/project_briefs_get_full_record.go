...
var projectBriefsGetFullRecordCmd = &cobra.Command{
	Use:   "get-full-record",
	Short: "Get a project brief",
	RunE:  runProjectBriefsGetFullRecord,
}

var projectBriefsGetFullRecordFlags struct {
...
func init() {
	projectBriefsGetFullRecordCmd.Flags().StringVar(&projectBriefsGetFullRecordFlags.projectBriefGid, "project-brief-gid", "", "Globally unique identifier for the project brief.")
	projectBriefsGetFullRecordCmd.MarkFlagRequired("project-brief-gid")
	projectBriefsGetFullRecordCmd.Flags().BoolVar(&projectBriefsGetFullRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	projectBriefsGetFullRecordCmd.Flags().StringSliceVar(&projectBriefsGetFullRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	projectBriefsCmd.AddCommand(projectBriefsGetFullRecordCmd)
}

func runProjectBriefsGetFullRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["project_brief_gid"] = fmt.Sprintf("%v", projectBriefsGetFullRecordFlags.projectBriefGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", projectBriefsGetFullRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = projectBriefsGetFullRecordFlags.optFields
	}
