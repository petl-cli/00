...
var sectionsUpdateSectionRecordCmd = &cobra.Command{
	Use:   "update-section-record",
	Short: "Update a section",
	RunE:  runSectionsUpdateSectionRecord,
}

var sectionsUpdateSectionRecordFlags struct {
	sectionGid string
	optPretty  bool
	optFields  []string
	body       string
...
func init() {
	sectionsUpdateSectionRecordCmd.Flags().StringVar(&sectionsUpdateSectionRecordFlags.sectionGid, "section-gid", "", "The globally unique identifier for the section.")
	sectionsUpdateSectionRecordCmd.MarkFlagRequired("section-gid")
	sectionsUpdateSectionRecordCmd.Flags().BoolVar(&sectionsUpdateSectionRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	sectionsUpdateSectionRecordCmd.Flags().StringSliceVar(&sectionsUpdateSectionRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	sectionsUpdateSectionRecordCmd.Flags().StringVar(&sectionsUpdateSectionRecordFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	sectionsCmd.AddCommand(sectionsUpdateSectionRecordCmd)
}

func runSectionsUpdateSectionRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["section_gid"] = fmt.Sprintf("%v", sectionsUpdateSectionRecordFlags.sectionGid)

	req := &httpclient.Request{
		Method:      "PUT",
		Path:        httpclient.SubstitutePath("/sections/{section_gid}", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", sectionsUpdateSectionRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = sectionsUpdateSectionRecordFlags.optFields
	}
	...
	if sectionsUpdateSectionRecordFlags.body != "" {
		if err := json.Unmarshal([]byte(sectionsUpdateSectionRecordFlags.body), &bodyMap); err != nil {
