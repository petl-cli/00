...
var webhooksRemoveWebhookCmd = &cobra.Command{
	Use:   "remove-webhook",
	Short: "Delete a webhook",
	RunE:  runWebhooksRemoveWebhook,
}

var webhooksRemoveWebhookFlags struct {
	webhookGid string
	optPretty  bool
}

func init() {
	webhooksRemoveWebhookCmd.Flags().StringVar(&webhooksRemoveWebhookFlags.webhookGid, "webhook-gid", "", "Globally unique identifier for the webhook.")
	webhooksRemoveWebhookCmd.MarkFlagRequired("webhook-gid")
	webhooksRemoveWebhookCmd.Flags().BoolVar(&webhooksRemoveWebhookFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")

	webhooksCmd.AddCommand(webhooksRemoveWebhookCmd)
}

func runWebhooksRemoveWebhook(cmd *cobra.Command, args []string) error {
	...
	pathParams["webhook_gid"] = fmt.Sprintf("%v", webhooksRemoveWebhookFlags.webhookGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", webhooksRemoveWebhookFlags.optPretty)
	}
