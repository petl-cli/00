...
var organizationExportsCreateExportRequestCmd = &cobra.Command{
	Use:   "create-export-request",
	Short: "Create an organization export request",
	RunE:  runOrganizationExportsCreateExportRequest,
}

var organizationExportsCreateExportRequestFlags struct {
	optPretty bool
	optFields []string
	body      string
}
...
func init() {
	organizationExportsCreateExportRequestCmd.Flags().BoolVar(&organizationExportsCreateExportRequestFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	organizationExportsCreateExportRequestCmd.Flags().StringSliceVar(&organizationExportsCreateExportRequestFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	organizationExportsCreateExportRequestCmd.Flags().StringVar(&organizationExportsCreateExportRequestFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	organizationExportsCmd.AddCommand(organizationExportsCreateExportRequestCmd)
}

func runOrganizationExportsCreateExportRequest(cmd *cobra.Command, args []string) error {
	...
	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/organization_exports", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", organizationExportsCreateExportRequestFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = organizationExportsCreateExportRequestFlags.optFields
	}
	...
	if organizationExportsCreateExportRequestFlags.body != "" {
		if err := json.Unmarshal([]byte(organizationExportsCreateExportRequestFlags.body), &bodyMap); err != nil {
