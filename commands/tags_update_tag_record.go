...
var tagsUpdateTagRecordCmd = &cobra.Command{
	Use:   "update-tag-record",
	Short: "Update a tag",
	RunE:  runTagsUpdateTagRecord,
}

var tagsUpdateTagRecordFlags struct {
	tagGid    string
	optPretty bool
	optFields []string
}
...
func init() {
	tagsUpdateTagRecordCmd.Flags().StringVar(&tagsUpdateTagRecordFlags.tagGid, "tag-gid", "", "Globally unique identifier for the tag.")
	tagsUpdateTagRecordCmd.MarkFlagRequired("tag-gid")
	tagsUpdateTagRecordCmd.Flags().BoolVar(&tagsUpdateTagRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	tagsUpdateTagRecordCmd.Flags().StringSliceVar(&tagsUpdateTagRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	tagsCmd.AddCommand(tagsUpdateTagRecordCmd)
}

func runTagsUpdateTagRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["tag_gid"] = fmt.Sprintf("%v", tagsUpdateTagRecordFlags.tagGid)

	req := &httpclient.Request{
		Method:      "PUT",
		Path:        httpclient.SubstitutePath("/tags/{tag_gid}", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", tagsUpdateTagRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = tagsUpdateTagRecordFlags.optFields
	}
