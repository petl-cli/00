...
var projectsAddCustomFieldSettingCmd = &cobra.Command{
	Use:   "add-custom-field-setting",
	Short: "Add a custom field to a project",
	RunE:  runProjectsAddCustomFieldSetting,
}

var projectsAddCustomFieldSettingFlags struct {
	projectGid string
	optPretty  bool
	optFields  []string
	body       string
...
func init() {
	projectsAddCustomFieldSettingCmd.Flags().StringVar(&projectsAddCustomFieldSettingFlags.projectGid, "project-gid", "", "Globally unique identifier for the project.")
	projectsAddCustomFieldSettingCmd.MarkFlagRequired("project-gid")
	projectsAddCustomFieldSettingCmd.Flags().BoolVar(&projectsAddCustomFieldSettingFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	projectsAddCustomFieldSettingCmd.Flags().StringSliceVar(&projectsAddCustomFieldSettingFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	projectsAddCustomFieldSettingCmd.Flags().StringVar(&projectsAddCustomFieldSettingFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	projectsCmd.AddCommand(projectsAddCustomFieldSettingCmd)
}

func runProjectsAddCustomFieldSetting(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "add-custom-field-setting",
	...
	pathParams["project_gid"] = fmt.Sprintf("%v", projectsAddCustomFieldSettingFlags.projectGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/projects/{project_gid}/addCustomFieldSetting", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", projectsAddCustomFieldSettingFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = projectsAddCustomFieldSettingFlags.optFields
	}
	...
	if projectsAddCustomFieldSettingFlags.body != "" {
		if err := json.Unmarshal([]byte(projectsAddCustomFieldSettingFlags.body), &bodyMap); err != nil {
