...
var tasksGetTasksByProjectCmd = &cobra.Command{
	Use:   "get-tasks-by-project",
	Short: "Get tasks from a project",
	RunE:  runTasksGetTasksByProject,
}

var tasksGetTasksByProjectFlags struct {
...
func init() {
	tasksGetTasksByProjectCmd.Flags().StringVar(&tasksGetTasksByProjectFlags.completedSince, "completed-since", "", "Only return tasks that are either incomplete or that have been completed since this time. Accepts a date-time string or the keyword *now*. ")
	tasksGetTasksByProjectCmd.Flags().StringVar(&tasksGetTasksByProjectFlags.projectGid, "project-gid", "", "Globally unique identifier for the project.")
	tasksGetTasksByProjectCmd.MarkFlagRequired("project-gid")
	tasksGetTasksByProjectCmd.Flags().BoolVar(&tasksGetTasksByProjectFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	tasksGetTasksByProjectCmd.Flags().IntVar(&tasksGetTasksByProjectFlags.limit, "limit", 0, "Results per page. The number of objects to return per page. The value must be between 1 and 100.")
	tasksGetTasksByProjectCmd.Flags().StringVar(&tasksGetTasksByProjectFlags.offset, "offset", "", "Offset token. An offset to the next page returned by the API. A pagination request will return an offset token, which can be used as an input parameter to the next request. If an offset is not passed in, the API will return the first page of results. *Note: You can only pass in an offset that was returned to you via a previously paginated request.*")
	tasksGetTasksByProjectCmd.Flags().StringSliceVar(&tasksGetTasksByProjectFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	tasksCmd.AddCommand(tasksGetTasksByProjectCmd)
}

func runTasksGetTasksByProject(cmd *cobra.Command, args []string) error {
	...
	pathParams["project_gid"] = fmt.Sprintf("%v", tasksGetTasksByProjectFlags.projectGid)

	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/projects/{project_gid}/tasks", pathParams),
	...
	if cmd.Flags().Changed("completed-since") {
		req.QueryParams["completed_since"] = fmt.Sprintf("%v", tasksGetTasksByProjectFlags.completedSince)
	}
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", tasksGetTasksByProjectFlags.optPretty)
	}
	if cmd.Flags().Changed("limit") {
		req.QueryParams["limit"] = fmt.Sprintf("%v", tasksGetTasksByProjectFlags.limit)
	}
	if cmd.Flags().Changed("offset") {
		req.QueryParams["offset"] = fmt.Sprintf("%v", tasksGetTasksByProjectFlags.offset)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = tasksGetTasksByProjectFlags.optFields
	}
