...
var teamsListWorkspaceTeamsCmd = &cobra.Command{
	Use:   "list-workspace-teams",
	Short: "Get teams in a workspace",
	RunE:  runTeamsListWorkspaceTeams,
}

var teamsListWorkspaceTeamsFlags struct {
	workspaceGid string
	optPretty    bool
	limit        int
	offset       string
...
func init() {
	teamsListWorkspaceTeamsCmd.Flags().StringVar(&teamsListWorkspaceTeamsFlags.workspaceGid, "workspace-gid", "", "Globally unique identifier for the workspace or organization.")
	teamsListWorkspaceTeamsCmd.MarkFlagRequired("workspace-gid")
	teamsListWorkspaceTeamsCmd.Flags().BoolVar(&teamsListWorkspaceTeamsFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	teamsListWorkspaceTeamsCmd.Flags().IntVar(&teamsListWorkspaceTeamsFlags.limit, "limit", 0, "Results per page. The number of objects to return per page. The value must be between 1 and 100.")
	teamsListWorkspaceTeamsCmd.Flags().StringVar(&teamsListWorkspaceTeamsFlags.offset, "offset", "", "Offset token. An offset to the next page returned by the API. A pagination request will return an offset token, which can be used as an input parameter to the next request. If an offset is not passed in, the API will return the first page of results. *Note: You can only pass in an offset that was returned to you via a previously paginated request.*")
	teamsListWorkspaceTeamsCmd.Flags().StringSliceVar(&teamsListWorkspaceTeamsFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	teamsCmd.AddCommand(teamsListWorkspaceTeamsCmd)
}

func runTeamsListWorkspaceTeams(cmd *cobra.Command, args []string) error {
	...
	pathParams["workspace_gid"] = fmt.Sprintf("%v", teamsListWorkspaceTeamsFlags.workspaceGid)

	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/workspaces/{workspace_gid}/teams", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", teamsListWorkspaceTeamsFlags.optPretty)
	}
	if cmd.Flags().Changed("limit") {
		req.QueryParams["limit"] = fmt.Sprintf("%v", teamsListWorkspaceTeamsFlags.limit)
	}
	if cmd.Flags().Changed("offset") {
		req.QueryParams["offset"] = fmt.Sprintf("%v", teamsListWorkspaceTeamsFlags.offset)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = teamsListWorkspaceTeamsFlags.optFields
	}
