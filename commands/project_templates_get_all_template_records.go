...
var projectTemplatesGetAllTemplateRecordsCmd = &cobra.Command{
	Use:   "get-all-template-records",
	Short: "Get a team's project templates",
	RunE:  runProjectTemplatesGetAllTemplateRecords,
}

var projectTemplatesGetAllTemplateRecordsFlags struct {
	teamGid   string
	optPretty bool
	limit     int
	offset    string
...
func init() {
	projectTemplatesGetAllTemplateRecordsCmd.Flags().StringVar(&projectTemplatesGetAllTemplateRecordsFlags.teamGid, "team-gid", "", "Globally unique identifier for the team.")
	projectTemplatesGetAllTemplateRecordsCmd.MarkFlagRequired("team-gid")
	projectTemplatesGetAllTemplateRecordsCmd.Flags().BoolVar(&projectTemplatesGetAllTemplateRecordsFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	projectTemplatesGetAllTemplateRecordsCmd.Flags().IntVar(&projectTemplatesGetAllTemplateRecordsFlags.limit, "limit", 0, "Results per page. The number of objects to return per page. The value must be between 1 and 100.")
	projectTemplatesGetAllTemplateRecordsCmd.Flags().StringVar(&projectTemplatesGetAllTemplateRecordsFlags.offset, "offset", "", "Offset token. An offset to the next page returned by the API. A pagination request will return an offset token, which can be used as an input parameter to the next request. If an offset is not passed in, the API will return the first page of results. *Note: You can only pass in an offset that was returned to you via a previously paginated request.*")
	projectTemplatesGetAllTemplateRecordsCmd.Flags().StringSliceVar(&projectTemplatesGetAllTemplateRecordsFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	projectTemplatesCmd.AddCommand(projectTemplatesGetAllTemplateRecordsCmd)
}

func runProjectTemplatesGetAllTemplateRecords(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "get-all-template-records",
	...
	pathParams["team_gid"] = fmt.Sprintf("%v", projectTemplatesGetAllTemplateRecordsFlags.teamGid)

	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/teams/{team_gid}/project_templates", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", projectTemplatesGetAllTemplateRecordsFlags.optPretty)
	}
	if cmd.Flags().Changed("limit") {
		req.QueryParams["limit"] = fmt.Sprintf("%v", projectTemplatesGetAllTemplateRecordsFlags.limit)
	}
	if cmd.Flags().Changed("offset") {
		req.QueryParams["offset"] = fmt.Sprintf("%v", projectTemplatesGetAllTemplateRecordsFlags.offset)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = projectTemplatesGetAllTemplateRecordsFlags.optFields
	}
