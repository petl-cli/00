...
var tasksUnlinkDependentsCmd = &cobra.Command{
	Use:   "unlink-dependents",
	Short: "Unlink dependents from a task",
	RunE:  runTasksUnlinkDependents,
}

var tasksUnlinkDependentsFlags struct {
	taskGid   string
	optPretty bool
	body      string
}
...
func init() {
	tasksUnlinkDependentsCmd.Flags().StringVar(&tasksUnlinkDependentsFlags.taskGid, "task-gid", "", "The task to operate on.")
	tasksUnlinkDependentsCmd.MarkFlagRequired("task-gid")
	tasksUnlinkDependentsCmd.Flags().BoolVar(&tasksUnlinkDependentsFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	tasksUnlinkDependentsCmd.Flags().StringVar(&tasksUnlinkDependentsFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	tasksCmd.AddCommand(tasksUnlinkDependentsCmd)
}

func runTasksUnlinkDependents(cmd *cobra.Command, args []string) error {
	...
	pathParams["task_gid"] = fmt.Sprintf("%v", tasksUnlinkDependentsFlags.taskGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/tasks/{task_gid}/removeDependents", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", tasksUnlinkDependentsFlags.optPretty)
	}
	...
	if tasksUnlinkDependentsFlags.body != "" {
		if err := json.Unmarshal([]byte(tasksUnlinkDependentsFlags.body), &bodyMap); err != nil {
