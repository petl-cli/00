...
var timeTrackingEntriesUpdateTimeTrackingEntryCmd = &cobra.Command{
	Use:   "update-time-tracking-entry",
	Short: "Update a time tracking entry",
	RunE:  runTimeTrackingEntriesUpdateTimeTrackingEntry,
}

var timeTrackingEntriesUpdateTimeTrackingEntryFlags struct {
...
func init() {
	timeTrackingEntriesUpdateTimeTrackingEntryCmd.Flags().StringVar(&timeTrackingEntriesUpdateTimeTrackingEntryFlags.timeTrackingEntryGid, "time-tracking-entry-gid", "", "Globally unique identifier for the time tracking entry.")
	timeTrackingEntriesUpdateTimeTrackingEntryCmd.MarkFlagRequired("time-tracking-entry-gid")
	timeTrackingEntriesUpdateTimeTrackingEntryCmd.Flags().BoolVar(&timeTrackingEntriesUpdateTimeTrackingEntryFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	timeTrackingEntriesUpdateTimeTrackingEntryCmd.Flags().StringSliceVar(&timeTrackingEntriesUpdateTimeTrackingEntryFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	timeTrackingEntriesUpdateTimeTrackingEntryCmd.Flags().StringVar(&timeTrackingEntriesUpdateTimeTrackingEntryFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	timeTrackingEntriesCmd.AddCommand(timeTrackingEntriesUpdateTimeTrackingEntryCmd)
}

func runTimeTrackingEntriesUpdateTimeTrackingEntry(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "update-time-tracking-entry",
	...
	pathParams["time_tracking_entry_gid"] = fmt.Sprintf("%v", timeTrackingEntriesUpdateTimeTrackingEntryFlags.timeTrackingEntryGid)

	req := &httpclient.Request{
		Method:      "PUT",
		Path:        httpclient.SubstitutePath("/time_tracking_entries/{time_tracking_entry_gid}", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", timeTrackingEntriesUpdateTimeTrackingEntryFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = timeTrackingEntriesUpdateTimeTrackingEntryFlags.optFields
	}
	...
	if timeTrackingEntriesUpdateTimeTrackingEntryFlags.body != "" {
		if err := json.Unmarshal([]byte(timeTrackingEntriesUpdateTimeTrackingEntryFlags.body), &bodyMap); err != nil {
