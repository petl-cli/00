...
var portfoliosCreateNewPortfolioRecordCmd = &cobra.Command{
	Use:   "create-new-portfolio-record",
	Short: "Create a portfolio",
	RunE:  runPortfoliosCreateNewPortfolioRecord,
}

var portfoliosCreateNewPortfolioRecordFlags struct {
	optPretty bool
	optFields []string
	data      string
	body      string
...
func init() {
	portfoliosCreateNewPortfolioRecordCmd.Flags().BoolVar(&portfoliosCreateNewPortfolioRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	portfoliosCreateNewPortfolioRecordCmd.Flags().StringSliceVar(&portfoliosCreateNewPortfolioRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	portfoliosCreateNewPortfolioRecordCmd.Flags().StringVar(&portfoliosCreateNewPortfolioRecordFlags.data, "data", "", "")
	// Note: body fields are not MarkFlagRequired — --body JSON satisfies them too.
	portfoliosCreateNewPortfolioRecordCmd.Flags().StringVar(&portfoliosCreateNewPortfolioRecordFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	portfoliosCmd.AddCommand(portfoliosCreateNewPortfolioRecordCmd)
}

func runPortfoliosCreateNewPortfolioRecord(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "create-new-portfolio-record",
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", portfoliosCreateNewPortfolioRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = portfoliosCreateNewPortfolioRecordFlags.optFields
	}
	...
	if portfoliosCreateNewPortfolioRecordFlags.body != "" {
		if err := json.Unmarshal([]byte(portfoliosCreateNewPortfolioRecordFlags.body), &bodyMap); err != nil {
	...
	if cmd.Flags().Changed("data") {
		bodyMap["data"] = portfoliosCreateNewPortfolioRecordFlags.data
	}
	req.Body = bodyMap
