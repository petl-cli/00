...
var allocationsGetMultipleCmd = &cobra.Command{
	Use:   "get-multiple",
	Short: "Get multiple allocations",
	RunE:  runAllocationsGetMultiple,
}

var allocationsGetMultipleFlags struct {
	optPretty bool
	parent    string
	assignee  string
	workspace string
...
func init() {
	allocationsGetMultipleCmd.Flags().BoolVar(&allocationsGetMultipleFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	allocationsGetMultipleCmd.Flags().StringVar(&allocationsGetMultipleFlags.parent, "parent", "", "Globally unique identifier for the project to filter allocations by.")
	allocationsGetMultipleCmd.Flags().StringVar(&allocationsGetMultipleFlags.assignee, "assignee", "", "Globally unique identifier for the user the allocation is assigned to.")
	allocationsGetMultipleCmd.Flags().StringVar(&allocationsGetMultipleFlags.workspace, "workspace", "", "Globally unique identifier for the workspace.")
	allocationsGetMultipleCmd.Flags().IntVar(&allocationsGetMultipleFlags.limit, "limit", 0, "Results per page. The number of objects to return per page. The value must be between 1 and 100.")
	allocationsGetMultipleCmd.Flags().StringVar(&allocationsGetMultipleFlags.offset, "offset", "", "Offset token. An offset to the next page returned by the API. A pagination request will return an offset token, which can be used as an input parameter to the next request. If an offset is not passed in, the API will return the first page of results. *Note: You can only pass in an offset that was returned to you via a previously paginated request.*")
	allocationsGetMultipleCmd.Flags().StringSliceVar(&allocationsGetMultipleFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	allocationsCmd.AddCommand(allocationsGetMultipleCmd)
}

func runAllocationsGetMultiple(cmd *cobra.Command, args []string) error {
	...
	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/allocations", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", allocationsGetMultipleFlags.optPretty)
	}
	if cmd.Flags().Changed("parent") {
		req.QueryParams["parent"] = fmt.Sprintf("%v", allocationsGetMultipleFlags.parent)
	}
	if cmd.Flags().Changed("assignee") {
		req.QueryParams["assignee"] = fmt.Sprintf("%v", allocationsGetMultipleFlags.assignee)
	}
	if cmd.Flags().Changed("workspace") {
		req.QueryParams["workspace"] = fmt.Sprintf("%v", allocationsGetMultipleFlags.workspace)
	}
	if cmd.Flags().Changed("limit") {
		req.QueryParams["limit"] = fmt.Sprintf("%v", allocationsGetMultipleFlags.limit)
	}
	if cmd.Flags().Changed("offset") {
		req.QueryParams["offset"] = fmt.Sprintf("%v", allocationsGetMultipleFlags.offset)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = allocationsGetMultipleFlags.optFields
	}
