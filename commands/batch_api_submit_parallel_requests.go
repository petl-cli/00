...
var batchApiSubmitParallelRequestsCmd = &cobra.Command{
	Use:   "submit-parallel-requests",
	Short: "Submit parallel requests",
	RunE:  runBatchApiSubmitParallelRequests,
}

var batchApiSubmitParallelRequestsFlags struct {
	optPretty bool
	optFields []string
	body      string
}
...
func init() {
	batchApiSubmitParallelRequestsCmd.Flags().BoolVar(&batchApiSubmitParallelRequestsFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	batchApiSubmitParallelRequestsCmd.Flags().StringSliceVar(&batchApiSubmitParallelRequestsFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	batchApiSubmitParallelRequestsCmd.Flags().StringVar(&batchApiSubmitParallelRequestsFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	batchApiCmd.AddCommand(batchApiSubmitParallelRequestsCmd)
}

func runBatchApiSubmitParallelRequests(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "submit-parallel-requests",
	...
	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/batch", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", batchApiSubmitParallelRequestsFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = batchApiSubmitParallelRequestsFlags.optFields
	}
	...
	if batchApiSubmitParallelRequestsFlags.body != "" {
		if err := json.Unmarshal([]byte(batchApiSubmitParallelRequestsFlags.body), &bodyMap); err != nil {
