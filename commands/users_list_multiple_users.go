...
var usersListMultipleUsersCmd = &cobra.Command{
	Use:   "list-multiple-users",
	Short: "Get multiple users",
	RunE:  runUsersListMultipleUsers,
}

var usersListMultipleUsersFlags struct {
	workspace string
	team      string
	optPretty bool
	limit     int
...
func init() {
	usersListMultipleUsersCmd.Flags().StringVar(&usersListMultipleUsersFlags.workspace, "workspace", "", "The workspace or organization ID to filter users on.")
	usersListMultipleUsersCmd.Flags().StringVar(&usersListMultipleUsersFlags.team, "team", "", "The team ID to filter users on.")
	usersListMultipleUsersCmd.Flags().BoolVar(&usersListMultipleUsersFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	usersListMultipleUsersCmd.Flags().IntVar(&usersListMultipleUsersFlags.limit, "limit", 0, "Results per page. The number of objects to return per page. The value must be between 1 and 100.")
	usersListMultipleUsersCmd.Flags().StringVar(&usersListMultipleUsersFlags.offset, "offset", "", "Offset token. An offset to the next page returned by the API. A pagination request will return an offset token, which can be used as an input parameter to the next request. If an offset is not passed in, the API will return the first page of results. *Note: You can only pass in an offset that was returned to you via a previously paginated request.*")
	usersListMultipleUsersCmd.Flags().StringSliceVar(&usersListMultipleUsersFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	usersCmd.AddCommand(usersListMultipleUsersCmd)
}

func runUsersListMultipleUsers(cmd *cobra.Command, args []string) error {
	...
	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/users", pathParams),
	...
	if cmd.Flags().Changed("workspace") {
		req.QueryParams["workspace"] = fmt.Sprintf("%v", usersListMultipleUsersFlags.workspace)
	}
	if cmd.Flags().Changed("team") {
		req.QueryParams["team"] = fmt.Sprintf("%v", usersListMultipleUsersFlags.team)
	}
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", usersListMultipleUsersFlags.optPretty)
	}
	if cmd.Flags().Changed("limit") {
		req.QueryParams["limit"] = fmt.Sprintf("%v", usersListMultipleUsersFlags.limit)
	}
	if cmd.Flags().Changed("offset") {
		req.QueryParams["offset"] = fmt.Sprintf("%v", usersListMultipleUsersFlags.offset)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = usersListMultipleUsersFlags.optFields
	}
