...
var timeTrackingEntriesGetRecordCmd = &cobra.Command{
	Use:   "get-record",
	Short: "Get a time tracking entry",
	RunE:  runTimeTrackingEntriesGetRecord,
}

var timeTrackingEntriesGetRecordFlags struct {
...
func init() {
	timeTrackingEntriesGetRecordCmd.Flags().StringVar(&timeTrackingEntriesGetRecordFlags.timeTrackingEntryGid, "time-tracking-entry-gid", "", "Globally unique identifier for the time tracking entry.")
	timeTrackingEntriesGetRecordCmd.MarkFlagRequired("time-tracking-entry-gid")
	timeTrackingEntriesGetRecordCmd.Flags().BoolVar(&timeTrackingEntriesGetRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	timeTrackingEntriesGetRecordCmd.Flags().StringSliceVar(&timeTrackingEntriesGetRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	timeTrackingEntriesCmd.AddCommand(timeTrackingEntriesGetRecordCmd)
}

func runTimeTrackingEntriesGetRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["time_tracking_entry_gid"] = fmt.Sprintf("%v", timeTrackingEntriesGetRecordFlags.timeTrackingEntryGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", timeTrackingEntriesGetRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = timeTrackingEntriesGetRecordFlags.optFields
	}
