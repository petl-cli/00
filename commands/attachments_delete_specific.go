...
var attachmentsDeleteSpecificCmd = &cobra.Command{
	Use:   "delete-specific",
	Short: "Delete an attachment",
	RunE:  runAttachmentsDeleteSpecific,
}

var attachmentsDeleteSpecificFlags struct {
	attachmentGid string
	optPretty     bool
}

func init() {
	attachmentsDeleteSpecificCmd.Flags().StringVar(&attachmentsDeleteSpecificFlags.attachmentGid, "attachment-gid", "", "Globally unique identifier for the attachment.")
	attachmentsDeleteSpecificCmd.MarkFlagRequired("attachment-gid")
	attachmentsDeleteSpecificCmd.Flags().BoolVar(&attachmentsDeleteSpecificFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")

	attachmentsCmd.AddCommand(attachmentsDeleteSpecificCmd)
}

func runAttachmentsDeleteSpecific(cmd *cobra.Command, args []string) error {
	...
	pathParams["attachment_gid"] = fmt.Sprintf("%v", attachmentsDeleteSpecificFlags.attachmentGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", attachmentsDeleteSpecificFlags.optPretty)
	}
