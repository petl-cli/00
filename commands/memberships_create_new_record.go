...
var membershipsCreateNewRecordCmd = &cobra.Command{
	Use:   "create-new-record",
	Short: "Create a membership",
	RunE:  runMembershipsCreateNewRecord,
}

var membershipsCreateNewRecordFlags struct {
	optPretty bool
	data      string
	body      string
}
...
func init() {
	membershipsCreateNewRecordCmd.Flags().BoolVar(&membershipsCreateNewRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	membershipsCreateNewRecordCmd.Flags().StringVar(&membershipsCreateNewRecordFlags.data, "data", "", "")
	// Note: body fields are not MarkFlagRequired — --body JSON satisfies them too.
	membershipsCreateNewRecordCmd.Flags().StringVar(&membershipsCreateNewRecordFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	membershipsCmd.AddCommand(membershipsCreateNewRecordCmd)
}

func runMembershipsCreateNewRecord(cmd *cobra.Command, args []string) error {
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", membershipsCreateNewRecordFlags.optPretty)
	}
	...
	if membershipsCreateNewRecordFlags.body != "" {
		if err := json.Unmarshal([]byte(membershipsCreateNewRecordFlags.body), &bodyMap); err != nil {
	...
	if cmd.Flags().Changed("data") {
		bodyMap["data"] = membershipsCreateNewRecordFlags.data
	}
	req.Body = bodyMap
