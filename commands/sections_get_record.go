...
var sectionsGetRecordCmd = &cobra.Command{
	Use:   "get-record",
	Short: "Get a section",
	RunE:  runSectionsGetRecord,
}

var sectionsGetRecordFlags struct {
	sectionGid string
	optPretty  bool
	optFields  []string
}
...
func init() {
	sectionsGetRecordCmd.Flags().StringVar(&sectionsGetRecordFlags.sectionGid, "section-gid", "", "The globally unique identifier for the section.")
	sectionsGetRecordCmd.MarkFlagRequired("section-gid")
	sectionsGetRecordCmd.Flags().BoolVar(&sectionsGetRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	sectionsGetRecordCmd.Flags().StringSliceVar(&sectionsGetRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	sectionsCmd.AddCommand(sectionsGetRecordCmd)
}

func runSectionsGetRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["section_gid"] = fmt.Sprintf("%v", sectionsGetRecordFlags.sectionGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", sectionsGetRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = sectionsGetRecordFlags.optFields
	}
