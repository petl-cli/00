...
var jobsGetByIdCmd = &cobra.Command{
	Use:   "get-by-id",
	Short: "Get a job by id",
	RunE:  runJobsGetById,
}

var jobsGetByIdFlags struct {
	jobGid    string
	optPretty bool
	optFields []string
}
...
func init() {
	jobsGetByIdCmd.Flags().StringVar(&jobsGetByIdFlags.jobGid, "job-gid", "", "Globally unique identifier for the job.")
	jobsGetByIdCmd.MarkFlagRequired("job-gid")
	jobsGetByIdCmd.Flags().BoolVar(&jobsGetByIdFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	jobsGetByIdCmd.Flags().StringSliceVar(&jobsGetByIdFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	jobsCmd.AddCommand(jobsGetByIdCmd)
}

func runJobsGetById(cmd *cobra.Command, args []string) error {
	...
	pathParams["job_gid"] = fmt.Sprintf("%v", jobsGetByIdFlags.jobGid)

	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/jobs/{job_gid}", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", jobsGetByIdFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = jobsGetByIdFlags.optFields
	}
