...
var projectStatusesDeleteSpecificStatusUpdateCmd = &cobra.Command{
	Use:   "delete-specific-status-update",
	Short: "Delete a project status",
	RunE:  runProjectStatusesDeleteSpecificStatusUpdate,
}

var projectStatusesDeleteSpecificStatusUpdateFlags struct {
	projectStatusGid string
	optPretty        bool
}

func init() {
	projectStatusesDeleteSpecificStatusUpdateCmd.Flags().StringVar(&projectStatusesDeleteSpecificStatusUpdateFlags.projectStatusGid, "project-status-gid", "", "The project status update to get.")
	projectStatusesDeleteSpecificStatusUpdateCmd.MarkFlagRequired("project-status-gid")
	projectStatusesDeleteSpecificStatusUpdateCmd.Flags().BoolVar(&projectStatusesDeleteSpecificStatusUpdateFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")

	projectStatusesCmd.AddCommand(projectStatusesDeleteSpecificStatusUpdateCmd)
}

func runProjectStatusesDeleteSpecificStatusUpdate(cmd *cobra.Command, args []string) error {
	...
	pathParams["project_status_gid"] = fmt.Sprintf("%v", projectStatusesDeleteSpecificStatusUpdateFlags.projectStatusGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", projectStatusesDeleteSpecificStatusUpdateFlags.optPretty)
	}
