...
var projectsCreateProjectTemplateJobCmd = &cobra.Command{
	Use:   "create-project-template-job",
	Short: "Create a project template from a project",
	RunE:  runProjectsCreateProjectTemplateJob,
}

var projectsCreateProjectTemplateJobFlags struct {
	projectGid string
	optPretty  bool
	optFields  []string
	body       string
...
func init() {
	projectsCreateProjectTemplateJobCmd.Flags().StringVar(&projectsCreateProjectTemplateJobFlags.projectGid, "project-gid", "", "Globally unique identifier for the project.")
	projectsCreateProjectTemplateJobCmd.MarkFlagRequired("project-gid")
	projectsCreateProjectTemplateJobCmd.Flags().BoolVar(&projectsCreateProjectTemplateJobFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	projectsCreateProjectTemplateJobCmd.Flags().StringSliceVar(&projectsCreateProjectTemplateJobFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	projectsCreateProjectTemplateJobCmd.Flags().StringVar(&projectsCreateProjectTemplateJobFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	projectsCmd.AddCommand(projectsCreateProjectTemplateJobCmd)
}

func runProjectsCreateProjectTemplateJob(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "create-project-template-job",
	...
	pathParams["project_gid"] = fmt.Sprintf("%v", projectsCreateProjectTemplateJobFlags.projectGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/projects/{project_gid}/saveAsTemplate", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", projectsCreateProjectTemplateJobFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = projectsCreateProjectTemplateJobFlags.optFields
	}
	...
	if projectsCreateProjectTemplateJobFlags.body != "" {
		if err := json.Unmarshal([]byte(projectsCreateProjectTemplateJobFlags.body), &bodyMap); err != nil {
