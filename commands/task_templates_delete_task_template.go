...
var taskTemplatesDeleteTaskTemplateCmd = &cobra.Command{
	Use:   "delete-task-template",
	Short: "Delete a task template",
	RunE:  runTaskTemplatesDeleteTaskTemplate,
}

var taskTemplatesDeleteTaskTemplateFlags struct {
	taskTemplateGid string
	optPretty       bool
}

func init() {
	taskTemplatesDeleteTaskTemplateCmd.Flags().StringVar(&taskTemplatesDeleteTaskTemplateFlags.taskTemplateGid, "task-template-gid", "", "Globally unique identifier for the task template.")
	taskTemplatesDeleteTaskTemplateCmd.MarkFlagRequired("task-template-gid")
	taskTemplatesDeleteTaskTemplateCmd.Flags().BoolVar(&taskTemplatesDeleteTaskTemplateFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")

	taskTemplatesCmd.AddCommand(taskTemplatesDeleteTaskTemplateCmd)
}

func runTaskTemplatesDeleteTaskTemplate(cmd *cobra.Command, args []string) error {
	...
	pathParams["task_template_gid"] = fmt.Sprintf("%v", taskTemplatesDeleteTaskTemplateFlags.taskTemplateGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", taskTemplatesDeleteTaskTemplateFlags.optPretty)
	}
