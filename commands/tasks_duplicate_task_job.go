...
var tasksDuplicateTaskJobCmd = &cobra.Command{
	Use:   "duplicate-task-job",
	Short: "Duplicate a task",
	RunE:  runTasksDuplicateTaskJob,
}

var tasksDuplicateTaskJobFlags struct {
	taskGid   string
	optPretty bool
	optFields []string
	body      string
...
func init() {
	tasksDuplicateTaskJobCmd.Flags().StringVar(&tasksDuplicateTaskJobFlags.taskGid, "task-gid", "", "The task to operate on.")
	tasksDuplicateTaskJobCmd.MarkFlagRequired("task-gid")
	tasksDuplicateTaskJobCmd.Flags().BoolVar(&tasksDuplicateTaskJobFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	tasksDuplicateTaskJobCmd.Flags().StringSliceVar(&tasksDuplicateTaskJobFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	tasksDuplicateTaskJobCmd.Flags().StringVar(&tasksDuplicateTaskJobFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	tasksCmd.AddCommand(tasksDuplicateTaskJobCmd)
}

func runTasksDuplicateTaskJob(cmd *cobra.Command, args []string) error {
	...
	pathParams["task_gid"] = fmt.Sprintf("%v", tasksDuplicateTaskJobFlags.taskGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/tasks/{task_gid}/duplicate", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", tasksDuplicateTaskJobFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = tasksDuplicateTaskJobFlags.optFields
	}
	...
	if tasksDuplicateTaskJobFlags.body != "" {
		if err := json.Unmarshal([]byte(tasksDuplicateTaskJobFlags.body), &bodyMap); err != nil {
