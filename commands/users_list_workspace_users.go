...
var usersListWorkspaceUsersCmd = &cobra.Command{
	Use:   "list-workspace-users",
	Short: "Get users in a workspace or organization",
	RunE:  runUsersListWorkspaceUsers,
}

var usersListWorkspaceUsersFlags struct {
	workspaceGid string
	optPretty    bool
	offset       string
	optFields    []string
...
func init() {
	usersListWorkspaceUsersCmd.Flags().StringVar(&usersListWorkspaceUsersFlags.workspaceGid, "workspace-gid", "", "Globally unique identifier for the workspace or organization.")
	usersListWorkspaceUsersCmd.MarkFlagRequired("workspace-gid")
	usersListWorkspaceUsersCmd.Flags().BoolVar(&usersListWorkspaceUsersFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	usersListWorkspaceUsersCmd.Flags().StringVar(&usersListWorkspaceUsersFlags.offset, "offset", "", "Offset token. An offset to the next page returned by the API. A pagination request will return an offset token, which can be used as an input parameter to the next request. If an offset is not passed in, the API will return the first page of results. *Note: You can only pass in an offset that was returned to you via a previously paginated request.*")
	usersListWorkspaceUsersCmd.Flags().StringSliceVar(&usersListWorkspaceUsersFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	usersCmd.AddCommand(usersListWorkspaceUsersCmd)
}

func runUsersListWorkspaceUsers(cmd *cobra.Command, args []string) error {
	...
	pathParams["workspace_gid"] = fmt.Sprintf("%v", usersListWorkspaceUsersFlags.workspaceGid)

	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/workspaces/{workspace_gid}/users", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", usersListWorkspaceUsersFlags.optPretty)
	}
	if cmd.Flags().Changed("offset") {
		req.QueryParams["offset"] = fmt.Sprintf("%v", usersListWorkspaceUsersFlags.offset)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = usersListWorkspaceUsersFlags.optFields
	}
