...
var projectTemplatesInstantiateProjectJobCmd = &cobra.Command{
	Use:   "instantiate-project-job",
	Short: "Instantiate a project from a project template",
	RunE:  runProjectTemplatesInstantiateProjectJob,
}

var projectTemplatesInstantiateProjectJobFlags struct {
...
func init() {
	projectTemplatesInstantiateProjectJobCmd.Flags().StringVar(&projectTemplatesInstantiateProjectJobFlags.projectTemplateGid, "project-template-gid", "", "Globally unique identifier for the project template.")
	projectTemplatesInstantiateProjectJobCmd.MarkFlagRequired("project-template-gid")
	projectTemplatesInstantiateProjectJobCmd.Flags().BoolVar(&projectTemplatesInstantiateProjectJobFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	projectTemplatesInstantiateProjectJobCmd.Flags().StringSliceVar(&projectTemplatesInstantiateProjectJobFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	projectTemplatesInstantiateProjectJobCmd.Flags().StringVar(&projectTemplatesInstantiateProjectJobFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	projectTemplatesCmd.AddCommand(projectTemplatesInstantiateProjectJobCmd)
}

func runProjectTemplatesInstantiateProjectJob(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "instantiate-project-job",
	...
	pathParams["project_template_gid"] = fmt.Sprintf("%v", projectTemplatesInstantiateProjectJobFlags.projectTemplateGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/project_templates/{project_template_gid}/instantiateProject", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", projectTemplatesInstantiateProjectJobFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = projectTemplatesInstantiateProjectJobFlags.optFields
	}
	...
	if projectTemplatesInstantiateProjectJobFlags.body != "" {
		if err := json.Unmarshal([]byte(projectTemplatesInstantiateProjectJobFlags.body), &bodyMap); err != nil {
