...
var tasksDeleteTaskCmd = &cobra.Command{
	Use:   "delete-task",
	Short: "Delete a task",
	RunE:  runTasksDeleteTask,
}

var tasksDeleteTaskFlags struct {
	taskGid   string
	optPretty bool
}

func init() {
	tasksDeleteTaskCmd.Flags().StringVar(&tasksDeleteTaskFlags.taskGid, "task-gid", "", "The task to operate on.")
	tasksDeleteTaskCmd.MarkFlagRequired("task-gid")
	tasksDeleteTaskCmd.Flags().BoolVar(&tasksDeleteTaskFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")

	tasksCmd.AddCommand(tasksDeleteTaskCmd)
}

func runTasksDeleteTask(cmd *cobra.Command, args []string) error {
	...
	pathParams["task_gid"] = fmt.Sprintf("%v", tasksDeleteTaskFlags.taskGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", tasksDeleteTaskFlags.optPretty)
	}
