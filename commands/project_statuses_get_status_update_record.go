...
var projectStatusesGetStatusUpdateRecordCmd = &cobra.Command{
	Use:   "get-status-update-record",
	Short: "Get a project status",
	RunE:  runProjectStatusesGetStatusUpdateRecord,
}

var projectStatusesGetStatusUpdateRecordFlags struct {
...
func init() {
	projectStatusesGetStatusUpdateRecordCmd.Flags().StringVar(&projectStatusesGetStatusUpdateRecordFlags.projectStatusGid, "project-status-gid", "", "The project status update to get.")
	projectStatusesGetStatusUpdateRecordCmd.MarkFlagRequired("project-status-gid")
	projectStatusesGetStatusUpdateRecordCmd.Flags().BoolVar(&projectStatusesGetStatusUpdateRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	projectStatusesGetStatusUpdateRecordCmd.Flags().StringSliceVar(&projectStatusesGetStatusUpdateRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	projectStatusesCmd.AddCommand(projectStatusesGetStatusUpdateRecordCmd)
}

func runProjectStatusesGetStatusUpdateRecord(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "get-status-update-record",
	...
	pathParams["project_status_gid"] = fmt.Sprintf("%v", projectStatusesGetStatusUpdateRecordFlags.projectStatusGid)

	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/project_statuses/{project_status_gid}", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", projectStatusesGetStatusUpdateRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = projectStatusesGetStatusUpdateRecordFlags.optFields
	}
