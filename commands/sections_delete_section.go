...
var sectionsDeleteSectionCmd = &cobra.Command{
	Use:   "delete-section",
	Short: "Delete a section",
	RunE:  runSectionsDeleteSection,
}

var sectionsDeleteSectionFlags struct {
	sectionGid string
	optPretty  bool
}

func init() {
	sectionsDeleteSectionCmd.Flags().StringVar(&sectionsDeleteSectionFlags.sectionGid, "section-gid", "", "The globally unique identifier for the section.")
	sectionsDeleteSectionCmd.MarkFlagRequired("section-gid")
	sectionsDeleteSectionCmd.Flags().BoolVar(&sectionsDeleteSectionFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")

	sectionsCmd.AddCommand(sectionsDeleteSectionCmd)
}

func runSectionsDeleteSection(cmd *cobra.Command, args []string) error {
	...
	pathParams["section_gid"] = fmt.Sprintf("%v", sectionsDeleteSectionFlags.sectionGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", sectionsDeleteSectionFlags.optPretty)
	}
