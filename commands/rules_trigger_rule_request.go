...
var rulesTriggerRuleRequestCmd = &cobra.Command{
	Use:   "trigger-rule-request",
	Short: "Trigger a rule",
	RunE:  runRulesTriggerRuleRequest,
}

var rulesTriggerRuleRequestFlags struct {
	ruleTriggerGid string
	body           string
}

func init() {
	rulesTriggerRuleRequestCmd.Flags().StringVar(&rulesTriggerRuleRequestFlags.ruleTriggerGid, "rule-trigger-gid", "", "The ID of the incoming web request trigger. This value is a path parameter that is automatically generated for the API endpoint.")
	rulesTriggerRuleRequestCmd.MarkFlagRequired("rule-trigger-gid")
	rulesTriggerRuleRequestCmd.Flags().StringVar(&rulesTriggerRuleRequestFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	rulesCmd.AddCommand(rulesTriggerRuleRequestCmd)
}

func runRulesTriggerRuleRequest(cmd *cobra.Command, args []string) error {
	...
	pathParams["rule_trigger_gid"] = fmt.Sprintf("%v", rulesTriggerRuleRequestFlags.ruleTriggerGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/rule_triggers/{rule_trigger_gid}/run", pathParams),
	...
	if rulesTriggerRuleRequestFlags.body != "" {
		if err := json.Unmarshal([]byte(rulesTriggerRuleRequestFlags.body), &bodyMap); err != nil {
