...
var projectsDuplicateProjectJobCmd = &cobra.Command{
	Use:   "duplicate-project-job",
	Short: "Duplicate a project",
	RunE:  runProjectsDuplicateProjectJob,
}

var projectsDuplicateProjectJobFlags struct {
	projectGid string
	optPretty  bool
	optFields  []string
	body       string
...
func init() {
	projectsDuplicateProjectJobCmd.Flags().StringVar(&projectsDuplicateProjectJobFlags.projectGid, "project-gid", "", "Globally unique identifier for the project.")
	projectsDuplicateProjectJobCmd.MarkFlagRequired("project-gid")
	projectsDuplicateProjectJobCmd.Flags().BoolVar(&projectsDuplicateProjectJobFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	projectsDuplicateProjectJobCmd.Flags().StringSliceVar(&projectsDuplicateProjectJobFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	projectsDuplicateProjectJobCmd.Flags().StringVar(&projectsDuplicateProjectJobFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	projectsCmd.AddCommand(projectsDuplicateProjectJobCmd)
}

func runProjectsDuplicateProjectJob(cmd *cobra.Command, args []string) error {
	...
	pathParams["project_gid"] = fmt.Sprintf("%v", projectsDuplicateProjectJobFlags.projectGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/projects/{project_gid}/duplicate", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", projectsDuplicateProjectJobFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = projectsDuplicateProjectJobFlags.optFields
	}
	...
	if projectsDuplicateProjectJobFlags.body != "" {
		if err := json.Unmarshal([]byte(projectsDuplicateProjectJobFlags.body), &bodyMap); err != nil {
