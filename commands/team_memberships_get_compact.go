...
var teamMembershipsGetCompactCmd = &cobra.Command{
	Use:   "get-compact",
	Short: "Get memberships from a team",
	RunE:  runTeamMembershipsGetCompact,
}

var teamMembershipsGetCompactFlags struct {
	teamGid   string
	optPretty bool
	limit     int
	offset    string
...
func init() {
	teamMembershipsGetCompactCmd.Flags().StringVar(&teamMembershipsGetCompactFlags.teamGid, "team-gid", "", "Globally unique identifier for the team.")
	teamMembershipsGetCompactCmd.MarkFlagRequired("team-gid")
	teamMembershipsGetCompactCmd.Flags().BoolVar(&teamMembershipsGetCompactFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	teamMembershipsGetCompactCmd.Flags().IntVar(&teamMembershipsGetCompactFlags.limit, "limit", 0, "Results per page. The number of objects to return per page. The value must be between 1 and 100.")
	teamMembershipsGetCompactCmd.Flags().StringVar(&teamMembershipsGetCompactFlags.offset, "offset", "", "Offset token. An offset to the next page returned by the API. A pagination request will return an offset token, which can be used as an input parameter to the next request. If an offset is not passed in, the API will return the first page of results. *Note: You can only pass in an offset that was returned to you via a previously paginated request.*")
	teamMembershipsGetCompactCmd.Flags().StringSliceVar(&teamMembershipsGetCompactFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	teamMembershipsCmd.AddCommand(teamMembershipsGetCompactCmd)
}

func runTeamMembershipsGetCompact(cmd *cobra.Command, args []string) error {
	...
	pathParams["team_gid"] = fmt.Sprintf("%v", teamMembershipsGetCompactFlags.teamGid)

	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/teams/{team_gid}/team_memberships", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", teamMembershipsGetCompactFlags.optPretty)
	}
	if cmd.Flags().Changed("limit") {
		req.QueryParams["limit"] = fmt.Sprintf("%v", teamMembershipsGetCompactFlags.limit)
	}
	if cmd.Flags().Changed("offset") {
		req.QueryParams["offset"] = fmt.Sprintf("%v", teamMembershipsGetCompactFlags.offset)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = teamMembershipsGetCompactFlags.optFields
	}
