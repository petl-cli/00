...
var tasksSetParentTaskCmd = &cobra.Command{
	Use:   "set-parent-task",
	Short: "Set the parent of a task",
	RunE:  runTasksSetParentTask,
}

var tasksSetParentTaskFlags struct {
	taskGid   string
	optPretty bool
	optFields []string
	body      string
...
func init() {
	tasksSetParentTaskCmd.Flags().StringVar(&tasksSetParentTaskFlags.taskGid, "task-gid", "", "The task to operate on.")
	tasksSetParentTaskCmd.MarkFlagRequired("task-gid")
	tasksSetParentTaskCmd.Flags().BoolVar(&tasksSetParentTaskFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	tasksSetParentTaskCmd.Flags().StringSliceVar(&tasksSetParentTaskFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	tasksSetParentTaskCmd.Flags().StringVar(&tasksSetParentTaskFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	tasksCmd.AddCommand(tasksSetParentTaskCmd)
}

func runTasksSetParentTask(cmd *cobra.Command, args []string) error {
	...
	pathParams["task_gid"] = fmt.Sprintf("%v", tasksSetParentTaskFlags.taskGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/tasks/{task_gid}/setParent", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", tasksSetParentTaskFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = tasksSetParentTaskFlags.optFields
	}
	...
	if tasksSetParentTaskFlags.body != "" {
		if err := json.Unmarshal([]byte(tasksSetParentTaskFlags.body), &bodyMap); err != nil {
