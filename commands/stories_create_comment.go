...
var storiesCreateCommentCmd = &cobra.Command{
	Use:   "create-comment",
	Short: "Create a story on a task",
	RunE:  runStoriesCreateComment,
}

var storiesCreateCommentFlags struct {
	taskGid   string
	optPretty bool
	optFields []string
	data      string
...
func init() {
	storiesCreateCommentCmd.Flags().StringVar(&storiesCreateCommentFlags.taskGid, "task-gid", "", "The task to operate on.")
	storiesCreateCommentCmd.MarkFlagRequired("task-gid")
	storiesCreateCommentCmd.Flags().BoolVar(&storiesCreateCommentFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	storiesCreateCommentCmd.Flags().StringSliceVar(&storiesCreateCommentFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	storiesCreateCommentCmd.Flags().StringVar(&storiesCreateCommentFlags.data, "data", "", "")
	// Note: body fields are not MarkFlagRequired — --body JSON satisfies them too.
	storiesCreateCommentCmd.Flags().StringVar(&storiesCreateCommentFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	storiesCmd.AddCommand(storiesCreateCommentCmd)
}

func runStoriesCreateComment(cmd *cobra.Command, args []string) error {
	...
	pathParams["task_gid"] = fmt.Sprintf("%v", storiesCreateCommentFlags.taskGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", storiesCreateCommentFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = storiesCreateCommentFlags.optFields
	}
	...
	if storiesCreateCommentFlags.body != "" {
		if err := json.Unmarshal([]byte(storiesCreateCommentFlags.body), &bodyMap); err != nil {
	...
	if cmd.Flags().Changed("data") {
		bodyMap["data"] = storiesCreateCommentFlags.data
	}
	req.Body = bodyMap
