...
var portfoliosGetItemsCmd = &cobra.Command{
	Use:   "get-items",
	Short: "Get portfolio items",
	RunE:  runPortfoliosGetItems,
}

var portfoliosGetItemsFlags struct {
	portfolioGid string
	optPretty    bool
	limit        int
	offset       string
...
func init() {
	portfoliosGetItemsCmd.Flags().StringVar(&portfoliosGetItemsFlags.portfolioGid, "portfolio-gid", "", "Globally unique identifier for the portfolio.")
	portfoliosGetItemsCmd.MarkFlagRequired("portfolio-gid")
	portfoliosGetItemsCmd.Flags().BoolVar(&portfoliosGetItemsFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	portfoliosGetItemsCmd.Flags().IntVar(&portfoliosGetItemsFlags.limit, "limit", 0, "Results per page. The number of objects to return per page. The value must be between 1 and 100.")
	portfoliosGetItemsCmd.Flags().StringVar(&portfoliosGetItemsFlags.offset, "offset", "", "Offset token. An offset to the next page returned by the API. A pagination request will return an offset token, which can be used as an input parameter to the next request. If an offset is not passed in, the API will return the first page of results. *Note: You can only pass in an offset that was returned to you via a previously paginated request.*")
	portfoliosGetItemsCmd.Flags().StringSliceVar(&portfoliosGetItemsFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	portfoliosCmd.AddCommand(portfoliosGetItemsCmd)
}

func runPortfoliosGetItems(cmd *cobra.Command, args []string) error {
	...
	pathParams["portfolio_gid"] = fmt.Sprintf("%v", portfoliosGetItemsFlags.portfolioGid)

	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/portfolios/{portfolio_gid}/items", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", portfoliosGetItemsFlags.optPretty)
	}
	if cmd.Flags().Changed("limit") {
		req.QueryParams["limit"] = fmt.Sprintf("%v", portfoliosGetItemsFlags.limit)
	}
	if cmd.Flags().Changed("offset") {
		req.QueryParams["offset"] = fmt.Sprintf("%v", portfoliosGetItemsFlags.offset)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = portfoliosGetItemsFlags.optFields
	}
