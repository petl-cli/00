...
var webhooksListMultipleWebhooksCmd = &cobra.Command{
	Use:   "list-multiple-webhooks",
	Short: "Get multiple webhooks",
	RunE:  runWebhooksListMultipleWebhooks,
}

var webhooksListMultipleWebhooksFlags struct {
	optPretty bool
	limit     int
	offset    string
	workspace string
...
func init() {
	webhooksListMultipleWebhooksCmd.Flags().BoolVar(&webhooksListMultipleWebhooksFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	webhooksListMultipleWebhooksCmd.Flags().IntVar(&webhooksListMultipleWebhooksFlags.limit, "limit", 0, "Results per page. The number of objects to return per page. The value must be between 1 and 100.")
	webhooksListMultipleWebhooksCmd.Flags().StringVar(&webhooksListMultipleWebhooksFlags.offset, "offset", "", "Offset token. An offset to the next page returned by the API. A pagination request will return an offset token, which can be used as an input parameter to the next request. If an offset is not passed in, the API will return the first page of results. *Note: You can only pass in an offset that was returned to you via a previously paginated request.*")
	webhooksListMultipleWebhooksCmd.Flags().StringVar(&webhooksListMultipleWebhooksFlags.workspace, "workspace", "", "The workspace to query for webhooks in.")
	webhooksListMultipleWebhooksCmd.MarkFlagRequired("workspace")
	webhooksListMultipleWebhooksCmd.Flags().StringVar(&webhooksListMultipleWebhooksFlags.resource, "resource", "", "Only return webhooks for the given resource.")
	webhooksListMultipleWebhooksCmd.Flags().StringSliceVar(&webhooksListMultipleWebhooksFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	webhooksCmd.AddCommand(webhooksListMultipleWebhooksCmd)
}

func runWebhooksListMultipleWebhooks(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "list-multiple-webhooks",
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", webhooksListMultipleWebhooksFlags.optPretty)
	}
	if cmd.Flags().Changed("limit") {
		req.QueryParams["limit"] = fmt.Sprintf("%v", webhooksListMultipleWebhooksFlags.limit)
	}
	if cmd.Flags().Changed("offset") {
		req.QueryParams["offset"] = fmt.Sprintf("%v", webhooksListMultipleWebhooksFlags.offset)
	}
	if cmd.Flags().Changed("workspace") {
		req.QueryParams["workspace"] = fmt.Sprintf("%v", webhooksListMultipleWebhooksFlags.workspace)
	}
	if cmd.Flags().Changed("resource") {
		req.QueryParams["resource"] = fmt.Sprintf("%v", webhooksListMultipleWebhooksFlags.resource)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = webhooksListMultipleWebhooksFlags.optFields
	}
