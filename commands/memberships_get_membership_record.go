...
var membershipsGetMembershipRecordCmd = &cobra.Command{
	Use:   "get-membership-record",
	Short: "Get a membership",
	RunE:  runMembershipsGetMembershipRecord,
}

var membershipsGetMembershipRecordFlags struct {
...
func init() {
	membershipsGetMembershipRecordCmd.Flags().StringVar(&membershipsGetMembershipRecordFlags.membershipGid, "membership-gid", "", "Globally unique identifier for the membership.")
	membershipsGetMembershipRecordCmd.MarkFlagRequired("membership-gid")
	membershipsGetMembershipRecordCmd.Flags().BoolVar(&membershipsGetMembershipRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	membershipsGetMembershipRecordCmd.Flags().StringSliceVar(&membershipsGetMembershipRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	membershipsCmd.AddCommand(membershipsGetMembershipRecordCmd)
}

func runMembershipsGetMembershipRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["membership_gid"] = fmt.Sprintf("%v", membershipsGetMembershipRecordFlags.membershipGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", membershipsGetMembershipRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = membershipsGetMembershipRecordFlags.optFields
	}
