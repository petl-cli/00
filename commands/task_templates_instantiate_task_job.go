...
var taskTemplatesInstantiateTaskJobCmd = &cobra.Command{
	Use:   "instantiate-task-job",
	Short: "Instantiate a task from a task template",
	RunE:  runTaskTemplatesInstantiateTaskJob,
}

var taskTemplatesInstantiateTaskJobFlags struct {
...
func init() {
	taskTemplatesInstantiateTaskJobCmd.Flags().StringVar(&taskTemplatesInstantiateTaskJobFlags.taskTemplateGid, "task-template-gid", "", "Globally unique identifier for the task template.")
	taskTemplatesInstantiateTaskJobCmd.MarkFlagRequired("task-template-gid")
	taskTemplatesInstantiateTaskJobCmd.Flags().BoolVar(&taskTemplatesInstantiateTaskJobFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	taskTemplatesInstantiateTaskJobCmd.Flags().StringSliceVar(&taskTemplatesInstantiateTaskJobFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	taskTemplatesInstantiateTaskJobCmd.Flags().StringVar(&taskTemplatesInstantiateTaskJobFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	taskTemplatesCmd.AddCommand(taskTemplatesInstantiateTaskJobCmd)
}

func runTaskTemplatesInstantiateTaskJob(cmd *cobra.Command, args []string) error {
	...
	pathParams["task_template_gid"] = fmt.Sprintf("%v", taskTemplatesInstantiateTaskJobFlags.taskTemplateGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/task_templates/{task_template_gid}/instantiateTask", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", taskTemplatesInstantiateTaskJobFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = taskTemplatesInstantiateTaskJobFlags.optFields
	}
	...
	if taskTemplatesInstantiateTaskJobFlags.body != "" {
		if err := json.Unmarshal([]byte(taskTemplatesInstantiateTaskJobFlags.body), &bodyMap); err != nil {
