...
var storiesGetFullRecordCmd = &cobra.Command{
	Use:   "get-full-record",
	Short: "Get a story",
	RunE:  runStoriesGetFullRecord,
}

var storiesGetFullRecordFlags struct {
	storyGid  string
	optPretty bool
	optFields []string
}
...
func init() {
	storiesGetFullRecordCmd.Flags().StringVar(&storiesGetFullRecordFlags.storyGid, "story-gid", "", "Globally unique identifier for the story.")
	storiesGetFullRecordCmd.MarkFlagRequired("story-gid")
	storiesGetFullRecordCmd.Flags().BoolVar(&storiesGetFullRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	storiesGetFullRecordCmd.Flags().StringSliceVar(&storiesGetFullRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	storiesCmd.AddCommand(storiesGetFullRecordCmd)
}

func runStoriesGetFullRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["story_gid"] = fmt.Sprintf("%v", storiesGetFullRecordFlags.storyGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", storiesGetFullRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = storiesGetFullRecordFlags.optFields
	}
