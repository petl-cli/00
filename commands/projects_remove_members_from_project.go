...
var projectsRemoveMembersFromProjectCmd = &cobra.Command{
	Use:   "remove-members-from-project",
	Short: "Remove users from a project",
	RunE:  runProjectsRemoveMembersFromProject,
}

var projectsRemoveMembersFromProjectFlags struct {
	projectGid string
	optPretty  bool
	optFields  []string
	body       string
...
func init() {
	projectsRemoveMembersFromProjectCmd.Flags().StringVar(&projectsRemoveMembersFromProjectFlags.projectGid, "project-gid", "", "Globally unique identifier for the project.")
	projectsRemoveMembersFromProjectCmd.MarkFlagRequired("project-gid")
	projectsRemoveMembersFromProjectCmd.Flags().BoolVar(&projectsRemoveMembersFromProjectFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	projectsRemoveMembersFromProjectCmd.Flags().StringSliceVar(&projectsRemoveMembersFromProjectFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	projectsRemoveMembersFromProjectCmd.Flags().StringVar(&projectsRemoveMembersFromProjectFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	projectsCmd.AddCommand(projectsRemoveMembersFromProjectCmd)
}

func runProjectsRemoveMembersFromProject(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "remove-members-from-project",
	...
	pathParams["project_gid"] = fmt.Sprintf("%v", projectsRemoveMembersFromProjectFlags.projectGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/projects/{project_gid}/removeMembers", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", projectsRemoveMembersFromProjectFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = projectsRemoveMembersFromProjectFlags.optFields
	}
	...
	if projectsRemoveMembersFromProjectFlags.body != "" {
		if err := json.Unmarshal([]byte(projectsRemoveMembersFromProjectFlags.body), &bodyMap); err != nil {
