...
var allocationsCreateRecordCmd = &cobra.Command{
	Use:   "create-record",
	Short: "Create an allocation",
	RunE:  runAllocationsCreateRecord,
}

var allocationsCreateRecordFlags struct {
	optPretty bool
	optFields []string
	data      string
	body      string
...
func init() {
	allocationsCreateRecordCmd.Flags().BoolVar(&allocationsCreateRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	allocationsCreateRecordCmd.Flags().StringSliceVar(&allocationsCreateRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	allocationsCreateRecordCmd.Flags().StringVar(&allocationsCreateRecordFlags.data, "data", "", "")
	// Note: body fields are not MarkFlagRequired — --body JSON satisfies them too.
	allocationsCreateRecordCmd.Flags().StringVar(&allocationsCreateRecordFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	allocationsCmd.AddCommand(allocationsCreateRecordCmd)
}

func runAllocationsCreateRecord(cmd *cobra.Command, args []string) error {
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", allocationsCreateRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = allocationsCreateRecordFlags.optFields
	}
	...
	if allocationsCreateRecordFlags.body != "" {
		if err := json.Unmarshal([]byte(allocationsCreateRecordFlags.body), &bodyMap); err != nil {
	...
	if cmd.Flags().Changed("data") {
		bodyMap["data"] = allocationsCreateRecordFlags.data
	}
	req.Body = bodyMap
