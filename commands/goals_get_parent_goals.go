...
var goalsGetParentGoalsCmd = &cobra.Command{
	Use:   "get-parent-goals",
	Short: "Get parent goals from a goal",
	RunE:  runGoalsGetParentGoals,
}

var goalsGetParentGoalsFlags struct {
	goalGid   string
	optPretty bool
	optFields []string
}
...
func init() {
	goalsGetParentGoalsCmd.Flags().StringVar(&goalsGetParentGoalsFlags.goalGid, "goal-gid", "", "Globally unique identifier for the goal.")
	goalsGetParentGoalsCmd.MarkFlagRequired("goal-gid")
	goalsGetParentGoalsCmd.Flags().BoolVar(&goalsGetParentGoalsFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	goalsGetParentGoalsCmd.Flags().StringSliceVar(&goalsGetParentGoalsFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	goalsCmd.AddCommand(goalsGetParentGoalsCmd)
}

func runGoalsGetParentGoals(cmd *cobra.Command, args []string) error {
	...
	pathParams["goal_gid"] = fmt.Sprintf("%v", goalsGetParentGoalsFlags.goalGid)

	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/goals/{goal_gid}/parentGoals", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", goalsGetParentGoalsFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = goalsGetParentGoalsFlags.optFields
	}
