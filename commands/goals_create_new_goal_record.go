...
var goalsCreateNewGoalRecordCmd = &cobra.Command{
	Use:   "create-new-goal-record",
	Short: "Create a goal",
	RunE:  runGoalsCreateNewGoalRecord,
}

var goalsCreateNewGoalRecordFlags struct {
	optPretty bool
	optFields []string
	data      string
	body      string
...
func init() {
	goalsCreateNewGoalRecordCmd.Flags().BoolVar(&goalsCreateNewGoalRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	goalsCreateNewGoalRecordCmd.Flags().StringSliceVar(&goalsCreateNewGoalRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	goalsCreateNewGoalRecordCmd.Flags().StringVar(&goalsCreateNewGoalRecordFlags.data, "data", "", "")
	// Note: body fields are not MarkFlagRequired — --body JSON satisfies them too.
	goalsCreateNewGoalRecordCmd.Flags().StringVar(&goalsCreateNewGoalRecordFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	goalsCmd.AddCommand(goalsCreateNewGoalRecordCmd)
}

func runGoalsCreateNewGoalRecord(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "create-new-goal-record",
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", goalsCreateNewGoalRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = goalsCreateNewGoalRecordFlags.optFields
	}
	...
	if goalsCreateNewGoalRecordFlags.body != "" {
		if err := json.Unmarshal([]byte(goalsCreateNewGoalRecordFlags.body), &bodyMap); err != nil {
	...
	if cmd.Flags().Changed("data") {
		bodyMap["data"] = goalsCreateNewGoalRecordFlags.data
	}
	req.Body = bodyMap
