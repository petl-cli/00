...
var allocationsDeleteAllocationByIdCmd = &cobra.Command{
	Use:   "delete-allocation-by-id",
	Short: "Delete an allocation",
	RunE:  runAllocationsDeleteAllocationById,
}

var allocationsDeleteAllocationByIdFlags struct {
	allocationGid string
	optPretty     bool
}

func init() {
	allocationsDeleteAllocationByIdCmd.Flags().StringVar(&allocationsDeleteAllocationByIdFlags.allocationGid, "allocation-gid", "", "Globally unique identifier for the allocation.")
	allocationsDeleteAllocationByIdCmd.MarkFlagRequired("allocation-gid")
	allocationsDeleteAllocationByIdCmd.Flags().BoolVar(&allocationsDeleteAllocationByIdFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")

	allocationsCmd.AddCommand(allocationsDeleteAllocationByIdCmd)
}

func runAllocationsDeleteAllocationById(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "delete-allocation-by-id",
			...
			"semantics": map[string]any{
				...
				"idempotent":   true,
				"reversible":   false,
				"side_effects": []string{"destroys_multiple_resources"},
				"impact":       "high",
			},
			"requires_auth": true,
		}
	...
	pathParams["allocation_gid"] = fmt.Sprintf("%v", allocationsDeleteAllocationByIdFlags.allocationGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", allocationsDeleteAllocationByIdFlags.optPretty)
	}
