...
var workspaceMembershipsGetUserMembershipsCmd = &cobra.Command{
	Use:   "get-user-memberships",
	Short: "Get workspace memberships for a user",
	RunE:  runWorkspaceMembershipsGetUserMemberships,
}

var workspaceMembershipsGetUserMembershipsFlags struct {
	userGid   string
	optPretty bool
	limit     int
	offset    string
...
func init() {
	workspaceMembershipsGetUserMembershipsCmd.Flags().StringVar(&workspaceMembershipsGetUserMembershipsFlags.userGid, "user-gid", "", "A string identifying a user. This can either be the string \"me\", an email, or the gid of a user.")
	workspaceMembershipsGetUserMembershipsCmd.MarkFlagRequired("user-gid")
	workspaceMembershipsGetUserMembershipsCmd.Flags().BoolVar(&workspaceMembershipsGetUserMembershipsFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	workspaceMembershipsGetUserMembershipsCmd.Flags().IntVar(&workspaceMembershipsGetUserMembershipsFlags.limit, "limit", 0, "Results per page. The number of objects to return per page. The value must be between 1 and 100.")
	workspaceMembershipsGetUserMembershipsCmd.Flags().StringVar(&workspaceMembershipsGetUserMembershipsFlags.offset, "offset", "", "Offset token. An offset to the next page returned by the API. A pagination request will return an offset token, which can be used as an input parameter to the next request. If an offset is not passed in, the API will return the first page of results. *Note: You can only pass in an offset that was returned to you via a previously paginated request.*")
	workspaceMembershipsGetUserMembershipsCmd.Flags().StringSliceVar(&workspaceMembershipsGetUserMembershipsFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	workspaceMembershipsCmd.AddCommand(workspaceMembershipsGetUserMembershipsCmd)
}

func runWorkspaceMembershipsGetUserMemberships(cmd *cobra.Command, args []string) error {
	...
	pathParams["user_gid"] = fmt.Sprintf("%v", workspaceMembershipsGetUserMembershipsFlags.userGid)

	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/users/{user_gid}/workspace_memberships", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", workspaceMembershipsGetUserMembershipsFlags.optPretty)
	}
	if cmd.Flags().Changed("limit") {
		req.QueryParams["limit"] = fmt.Sprintf("%v", workspaceMembershipsGetUserMembershipsFlags.limit)
	}
	if cmd.Flags().Changed("offset") {
		req.QueryParams["offset"] = fmt.Sprintf("%v", workspaceMembershipsGetUserMembershipsFlags.offset)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = workspaceMembershipsGetUserMembershipsFlags.optFields
	}
