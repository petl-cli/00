...
var customFieldsReorderEnumOptionCmd = &cobra.Command{
	Use:   "reorder-enum-option",
	Short: "Reorder a custom field's enum",
	RunE:  runCustomFieldsReorderEnumOption,
}

var customFieldsReorderEnumOptionFlags struct {
...
func init() {
	customFieldsReorderEnumOptionCmd.Flags().StringVar(&customFieldsReorderEnumOptionFlags.customFieldGid, "custom-field-gid", "", "Globally unique identifier for the custom field.")
	customFieldsReorderEnumOptionCmd.MarkFlagRequired("custom-field-gid")
	customFieldsReorderEnumOptionCmd.Flags().BoolVar(&customFieldsReorderEnumOptionFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	customFieldsReorderEnumOptionCmd.Flags().StringSliceVar(&customFieldsReorderEnumOptionFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	customFieldsReorderEnumOptionCmd.Flags().StringVar(&customFieldsReorderEnumOptionFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	customFieldsCmd.AddCommand(customFieldsReorderEnumOptionCmd)
}

func runCustomFieldsReorderEnumOption(cmd *cobra.Command, args []string) error {
	...
	pathParams["custom_field_gid"] = fmt.Sprintf("%v", customFieldsReorderEnumOptionFlags.customFieldGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/custom_fields/{custom_field_gid}/enum_options/insert", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", customFieldsReorderEnumOptionFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = customFieldsReorderEnumOptionFlags.optFields
	}
	...
	if customFieldsReorderEnumOptionFlags.body != "" {
		if err := json.Unmarshal([]byte(customFieldsReorderEnumOptionFlags.body), &bodyMap); err != nil {
