...
var projectsRemoveProjectFollowersCmd = &cobra.Command{
	Use:   "remove-project-followers",
	Short: "Remove followers from a project",
	RunE:  runProjectsRemoveProjectFollowers,
}

var projectsRemoveProjectFollowersFlags struct {
	projectGid string
	optPretty  bool
	optFields  []string
	body       string
...
func init() {
	projectsRemoveProjectFollowersCmd.Flags().StringVar(&projectsRemoveProjectFollowersFlags.projectGid, "project-gid", "", "Globally unique identifier for the project.")
	projectsRemoveProjectFollowersCmd.MarkFlagRequired("project-gid")
	projectsRemoveProjectFollowersCmd.Flags().BoolVar(&projectsRemoveProjectFollowersFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	projectsRemoveProjectFollowersCmd.Flags().StringSliceVar(&projectsRemoveProjectFollowersFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	projectsRemoveProjectFollowersCmd.Flags().StringVar(&projectsRemoveProjectFollowersFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	projectsCmd.AddCommand(projectsRemoveProjectFollowersCmd)
}

func runProjectsRemoveProjectFollowers(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "remove-project-followers",
	...
	pathParams["project_gid"] = fmt.Sprintf("%v", projectsRemoveProjectFollowersFlags.projectGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/projects/{project_gid}/removeFollowers", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", projectsRemoveProjectFollowersFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = projectsRemoveProjectFollowersFlags.optFields
	}
	...
	if projectsRemoveProjectFollowersFlags.body != "" {
		if err := json.Unmarshal([]byte(projectsRemoveProjectFollowersFlags.body), &bodyMap); err != nil {
