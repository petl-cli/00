...
var membershipsDeleteRecordCmd = &cobra.Command{
	Use:   "delete-record",
	Short: "Delete a membership",
	RunE:  runMembershipsDeleteRecord,
}

var membershipsDeleteRecordFlags struct {
	membershipGid string
	optPretty     bool
}

func init() {
	membershipsDeleteRecordCmd.Flags().StringVar(&membershipsDeleteRecordFlags.membershipGid, "membership-gid", "", "Globally unique identifier for the membership.")
	membershipsDeleteRecordCmd.MarkFlagRequired("membership-gid")
	membershipsDeleteRecordCmd.Flags().BoolVar(&membershipsDeleteRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")

	membershipsCmd.AddCommand(membershipsDeleteRecordCmd)
}

func runMembershipsDeleteRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["membership_gid"] = fmt.Sprintf("%v", membershipsDeleteRecordFlags.membershipGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", membershipsDeleteRecordFlags.optPretty)
	}
