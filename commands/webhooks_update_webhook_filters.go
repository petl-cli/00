...
var webhooksUpdateWebhookFiltersCmd = &cobra.Command{
	Use:   "update-webhook-filters",
	Short: "Update a webhook",
	RunE:  runWebhooksUpdateWebhookFilters,
}

var webhooksUpdateWebhookFiltersFlags struct {
	webhookGid string
	optPretty  bool
	optFields  []string
	body       string
...
func init() {
	webhooksUpdateWebhookFiltersCmd.Flags().StringVar(&webhooksUpdateWebhookFiltersFlags.webhookGid, "webhook-gid", "", "Globally unique identifier for the webhook.")
	webhooksUpdateWebhookFiltersCmd.MarkFlagRequired("webhook-gid")
	webhooksUpdateWebhookFiltersCmd.Flags().BoolVar(&webhooksUpdateWebhookFiltersFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	webhooksUpdateWebhookFiltersCmd.Flags().StringSliceVar(&webhooksUpdateWebhookFiltersFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	webhooksUpdateWebhookFiltersCmd.Flags().StringVar(&webhooksUpdateWebhookFiltersFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	webhooksCmd.AddCommand(webhooksUpdateWebhookFiltersCmd)
}

func runWebhooksUpdateWebhookFilters(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "update-webhook-filters",
	...
	pathParams["webhook_gid"] = fmt.Sprintf("%v", webhooksUpdateWebhookFiltersFlags.webhookGid)

	req := &httpclient.Request{
		Method:      "PUT",
		Path:        httpclient.SubstitutePath("/webhooks/{webhook_gid}", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", webhooksUpdateWebhookFiltersFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = webhooksUpdateWebhookFiltersFlags.optFields
	}
	...
	if webhooksUpdateWebhookFiltersFlags.body != "" {
		if err := json.Unmarshal([]byte(webhooksUpdateWebhookFiltersFlags.body), &bodyMap); err != nil {
