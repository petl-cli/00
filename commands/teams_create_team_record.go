...
var teamsCreateTeamRecordCmd = &cobra.Command{
	Use:   "create-team-record",
	Short: "Create a team",
	RunE:  runTeamsCreateTeamRecord,
}

var teamsCreateTeamRecordFlags struct {
	optPretty bool
	optFields []string
	data      string
	body      string
...
func init() {
	teamsCreateTeamRecordCmd.Flags().BoolVar(&teamsCreateTeamRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	teamsCreateTeamRecordCmd.Flags().StringSliceVar(&teamsCreateTeamRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	teamsCreateTeamRecordCmd.Flags().StringVar(&teamsCreateTeamRecordFlags.data, "data", "", "")
	// Note: body fields are not MarkFlagRequired — --body JSON satisfies them too.
	teamsCreateTeamRecordCmd.Flags().StringVar(&teamsCreateTeamRecordFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	teamsCmd.AddCommand(teamsCreateTeamRecordCmd)
}

func runTeamsCreateTeamRecord(cmd *cobra.Command, args []string) error {
	...
	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/teams", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", teamsCreateTeamRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = teamsCreateTeamRecordFlags.optFields
	}
	...
	if teamsCreateTeamRecordFlags.body != "" {
		if err := json.Unmarshal([]byte(teamsCreateTeamRecordFlags.body), &bodyMap); err != nil {
	...
	if cmd.Flags().Changed("data") {
		bodyMap["data"] = teamsCreateTeamRecordFlags.data
	}
	req.Body = bodyMap
