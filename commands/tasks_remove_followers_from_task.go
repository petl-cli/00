...
var tasksRemoveFollowersFromTaskCmd = &cobra.Command{
	Use:   "remove-followers-from-task",
	Short: "Remove followers from a task",
	RunE:  runTasksRemoveFollowersFromTask,
}

var tasksRemoveFollowersFromTaskFlags struct {
	taskGid   string
	optPretty bool
	optFields []string
	body      string
...
func init() {
	tasksRemoveFollowersFromTaskCmd.Flags().StringVar(&tasksRemoveFollowersFromTaskFlags.taskGid, "task-gid", "", "The task to operate on.")
	tasksRemoveFollowersFromTaskCmd.MarkFlagRequired("task-gid")
	tasksRemoveFollowersFromTaskCmd.Flags().BoolVar(&tasksRemoveFollowersFromTaskFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	tasksRemoveFollowersFromTaskCmd.Flags().StringSliceVar(&tasksRemoveFollowersFromTaskFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	tasksRemoveFollowersFromTaskCmd.Flags().StringVar(&tasksRemoveFollowersFromTaskFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	tasksCmd.AddCommand(tasksRemoveFollowersFromTaskCmd)
}

func runTasksRemoveFollowersFromTask(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "remove-followers-from-task",
	...
	pathParams["task_gid"] = fmt.Sprintf("%v", tasksRemoveFollowersFromTaskFlags.taskGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/tasks/{task_gid}/removeFollowers", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", tasksRemoveFollowersFromTaskFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = tasksRemoveFollowersFromTaskFlags.optFields
	}
	...
	if tasksRemoveFollowersFromTaskFlags.body != "" {
		if err := json.Unmarshal([]byte(tasksRemoveFollowersFromTaskFlags.body), &bodyMap); err != nil {
