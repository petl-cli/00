...
var goalsGetGoalRecordCmd = &cobra.Command{
	Use:   "get-goal-record",
	Short: "Get a goal",
	RunE:  runGoalsGetGoalRecord,
}

var goalsGetGoalRecordFlags struct {
	goalGid   string
	optPretty bool
	optFields []string
}
...
func init() {
	goalsGetGoalRecordCmd.Flags().StringVar(&goalsGetGoalRecordFlags.goalGid, "goal-gid", "", "Globally unique identifier for the goal.")
	goalsGetGoalRecordCmd.MarkFlagRequired("goal-gid")
	goalsGetGoalRecordCmd.Flags().BoolVar(&goalsGetGoalRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	goalsGetGoalRecordCmd.Flags().StringSliceVar(&goalsGetGoalRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	goalsCmd.AddCommand(goalsGetGoalRecordCmd)
}

func runGoalsGetGoalRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["goal_gid"] = fmt.Sprintf("%v", goalsGetGoalRecordFlags.goalGid)

	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/goals/{goal_gid}", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", goalsGetGoalRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = goalsGetGoalRecordFlags.optFields
	}
