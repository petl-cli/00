...
var portfoliosListMultiplePortfoliosCmd = &cobra.Command{
	Use:   "list-multiple-portfolios",
	Short: "Get multiple portfolios",
	RunE:  runPortfoliosListMultiplePortfolios,
}

var portfoliosListMultiplePortfoliosFlags struct {
	optPretty bool
	limit     int
	offset    string
	workspace string
...
func init() {
	portfoliosListMultiplePortfoliosCmd.Flags().BoolVar(&portfoliosListMultiplePortfoliosFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	portfoliosListMultiplePortfoliosCmd.Flags().IntVar(&portfoliosListMultiplePortfoliosFlags.limit, "limit", 0, "Results per page. The number of objects to return per page. The value must be between 1 and 100.")
	portfoliosListMultiplePortfoliosCmd.Flags().StringVar(&portfoliosListMultiplePortfoliosFlags.offset, "offset", "", "Offset token. An offset to the next page returned by the API. A pagination request will return an offset token, which can be used as an input parameter to the next request. If an offset is not passed in, the API will return the first page of results. *Note: You can only pass in an offset that was returned to you via a previously paginated request.*")
	portfoliosListMultiplePortfoliosCmd.Flags().StringVar(&portfoliosListMultiplePortfoliosFlags.workspace, "workspace", "", "The workspace or organization to filter portfolios on.")
	portfoliosListMultiplePortfoliosCmd.MarkFlagRequired("workspace")
	portfoliosListMultiplePortfoliosCmd.Flags().StringVar(&portfoliosListMultiplePortfoliosFlags.owner, "owner", "", "The user who owns the portfolio. Currently, API users can only get a list of portfolios that they themselves own, unless the request is made from a Service Account. In the case of a Service Account, if this parameter is specified, then all portfolios owned by this parameter are returned. Otherwise, all portfolios across the workspace are returned.")
	portfoliosListMultiplePortfoliosCmd.Flags().StringSliceVar(&portfoliosListMultiplePortfoliosFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	portfoliosCmd.AddCommand(portfoliosListMultiplePortfoliosCmd)
}

func runPortfoliosListMultiplePortfolios(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "list-multiple-portfolios",
	...
	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/portfolios", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", portfoliosListMultiplePortfoliosFlags.optPretty)
	}
	if cmd.Flags().Changed("limit") {
		req.QueryParams["limit"] = fmt.Sprintf("%v", portfoliosListMultiplePortfoliosFlags.limit)
	}
	if cmd.Flags().Changed("offset") {
		req.QueryParams["offset"] = fmt.Sprintf("%v", portfoliosListMultiplePortfoliosFlags.offset)
	}
	if cmd.Flags().Changed("workspace") {
		req.QueryParams["workspace"] = fmt.Sprintf("%v", portfoliosListMultiplePortfoliosFlags.workspace)
	}
	if cmd.Flags().Changed("owner") {
		req.QueryParams["owner"] = fmt.Sprintf("%v", portfoliosListMultiplePortfoliosFlags.owner)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = portfoliosListMultiplePortfoliosFlags.optFields
	}
