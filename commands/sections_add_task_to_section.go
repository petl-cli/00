...
var sectionsAddTaskToSectionCmd = &cobra.Command{
	Use:   "add-task-to-section",
	Short: "Add task to section",
	RunE:  runSectionsAddTaskToSection,
}

var sectionsAddTaskToSectionFlags struct {
	sectionGid string
	optPretty  bool
	body       string
}
...
func init() {
	sectionsAddTaskToSectionCmd.Flags().StringVar(&sectionsAddTaskToSectionFlags.sectionGid, "section-gid", "", "The globally unique identifier for the section.")
	sectionsAddTaskToSectionCmd.MarkFlagRequired("section-gid")
	sectionsAddTaskToSectionCmd.Flags().BoolVar(&sectionsAddTaskToSectionFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	sectionsAddTaskToSectionCmd.Flags().StringVar(&sectionsAddTaskToSectionFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	sectionsCmd.AddCommand(sectionsAddTaskToSectionCmd)
}

func runSectionsAddTaskToSection(cmd *cobra.Command, args []string) error {
	...
	pathParams["section_gid"] = fmt.Sprintf("%v", sectionsAddTaskToSectionFlags.sectionGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/sections/{section_gid}/addTask", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", sectionsAddTaskToSectionFlags.optPretty)
	}
	...
	if sectionsAddTaskToSectionFlags.body != "" {
		if err := json.Unmarshal([]byte(sectionsAddTaskToSectionFlags.body), &bodyMap); err != nil {
