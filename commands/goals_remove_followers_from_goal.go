...
var goalsRemoveFollowersFromGoalCmd = &cobra.Command{
	Use:   "remove-followers-from-goal",
	Short: "Remove a collaborator from a goal",
	RunE:  runGoalsRemoveFollowersFromGoal,
}

var goalsRemoveFollowersFromGoalFlags struct {
	goalGid   string
	optPretty bool
	optFields []string
	body      string
...
func init() {
	goalsRemoveFollowersFromGoalCmd.Flags().StringVar(&goalsRemoveFollowersFromGoalFlags.goalGid, "goal-gid", "", "Globally unique identifier for the goal.")
	goalsRemoveFollowersFromGoalCmd.MarkFlagRequired("goal-gid")
	goalsRemoveFollowersFromGoalCmd.Flags().BoolVar(&goalsRemoveFollowersFromGoalFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	goalsRemoveFollowersFromGoalCmd.Flags().StringSliceVar(&goalsRemoveFollowersFromGoalFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	goalsRemoveFollowersFromGoalCmd.Flags().StringVar(&goalsRemoveFollowersFromGoalFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	goalsCmd.AddCommand(goalsRemoveFollowersFromGoalCmd)
}

func runGoalsRemoveFollowersFromGoal(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "remove-followers-from-goal",
	...
	pathParams["goal_gid"] = fmt.Sprintf("%v", goalsRemoveFollowersFromGoalFlags.goalGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/goals/{goal_gid}/removeFollowers", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", goalsRemoveFollowersFromGoalFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = goalsRemoveFollowersFromGoalFlags.optFields
	}
	...
	if goalsRemoveFollowersFromGoalFlags.body != "" {
		if err := json.Unmarshal([]byte(goalsRemoveFollowersFromGoalFlags.body), &bodyMap); err != nil {
