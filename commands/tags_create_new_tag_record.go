...
var tagsCreateNewTagRecordCmd = &cobra.Command{
	Use:   "create-new-tag-record",
	Short: "Create a tag",
	RunE:  runTagsCreateNewTagRecord,
}

var tagsCreateNewTagRecordFlags struct {
	optPretty bool
	optFields []string
	data      string
	body      string
...
func init() {
	tagsCreateNewTagRecordCmd.Flags().BoolVar(&tagsCreateNewTagRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	tagsCreateNewTagRecordCmd.Flags().StringSliceVar(&tagsCreateNewTagRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	tagsCreateNewTagRecordCmd.Flags().StringVar(&tagsCreateNewTagRecordFlags.data, "data", "", "")
	// Note: body fields are not MarkFlagRequired — --body JSON satisfies them too.
	tagsCreateNewTagRecordCmd.Flags().StringVar(&tagsCreateNewTagRecordFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	tagsCmd.AddCommand(tagsCreateNewTagRecordCmd)
}

func runTagsCreateNewTagRecord(cmd *cobra.Command, args []string) error {
	...
	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/tags", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", tagsCreateNewTagRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = tagsCreateNewTagRecordFlags.optFields
	}
	...
	if tagsCreateNewTagRecordFlags.body != "" {
		if err := json.Unmarshal([]byte(tagsCreateNewTagRecordFlags.body), &bodyMap); err != nil {
	...
	if cmd.Flags().Changed("data") {
		bodyMap["data"] = tagsCreateNewTagRecordFlags.data
	}
	req.Body = bodyMap
