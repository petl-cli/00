...
var timeTrackingEntriesDeleteTimeEntryCmd = &cobra.Command{
	Use:   "delete-time-entry",
	Short: "Delete a time tracking entry",
	RunE:  runTimeTrackingEntriesDeleteTimeEntry,
}

var timeTrackingEntriesDeleteTimeEntryFlags struct {
	timeTrackingEntryGid string
	optPretty            bool
}

func init() {
	timeTrackingEntriesDeleteTimeEntryCmd.Flags().StringVar(&timeTrackingEntriesDeleteTimeEntryFlags.timeTrackingEntryGid, "time-tracking-entry-gid", "", "Globally unique identifier for the time tracking entry.")
	timeTrackingEntriesDeleteTimeEntryCmd.MarkFlagRequired("time-tracking-entry-gid")
	timeTrackingEntriesDeleteTimeEntryCmd.Flags().BoolVar(&timeTrackingEntriesDeleteTimeEntryFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")

	timeTrackingEntriesCmd.AddCommand(timeTrackingEntriesDeleteTimeEntryCmd)
}

func runTimeTrackingEntriesDeleteTimeEntry(cmd *cobra.Command, args []string) error {
	...
	pathParams["time_tracking_entry_gid"] = fmt.Sprintf("%v", timeTrackingEntriesDeleteTimeEntryFlags.timeTrackingEntryGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", timeTrackingEntriesDeleteTimeEntryFlags.optPretty)
	}
