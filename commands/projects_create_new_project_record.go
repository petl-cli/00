...
var projectsCreateNewProjectRecordCmd = &cobra.Command{
	Use:   "create-new-project-record",
	Short: "Create a project",
	RunE:  runProjectsCreateNewProjectRecord,
}

var projectsCreateNewProjectRecordFlags struct {
	optPretty bool
	optFields []string
	data      string
	body      string
...
func init() {
	projectsCreateNewProjectRecordCmd.Flags().BoolVar(&projectsCreateNewProjectRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	projectsCreateNewProjectRecordCmd.Flags().StringSliceVar(&projectsCreateNewProjectRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	projectsCreateNewProjectRecordCmd.Flags().StringVar(&projectsCreateNewProjectRecordFlags.data, "data", "", "")
	// Note: body fields are not MarkFlagRequired — --body JSON satisfies them too.
	projectsCreateNewProjectRecordCmd.Flags().StringVar(&projectsCreateNewProjectRecordFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	projectsCmd.AddCommand(projectsCreateNewProjectRecordCmd)
}

func runProjectsCreateNewProjectRecord(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "create-new-project-record",
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", projectsCreateNewProjectRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = projectsCreateNewProjectRecordFlags.optFields
	}
	...
	if projectsCreateNewProjectRecordFlags.body != "" {
		if err := json.Unmarshal([]byte(projectsCreateNewProjectRecordFlags.body), &bodyMap); err != nil {
	...
	if cmd.Flags().Changed("data") {
		bodyMap["data"] = projectsCreateNewProjectRecordFlags.data
	}
	req.Body = bodyMap
