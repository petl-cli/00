...
var customFieldsAddEnumOptionCmd = &cobra.Command{
	Use:   "add-enum-option",
	Short: "Create an enum option",
	RunE:  runCustomFieldsAddEnumOption,
}

var customFieldsAddEnumOptionFlags struct {
...
func init() {
	customFieldsAddEnumOptionCmd.Flags().StringVar(&customFieldsAddEnumOptionFlags.customFieldGid, "custom-field-gid", "", "Globally unique identifier for the custom field.")
	customFieldsAddEnumOptionCmd.MarkFlagRequired("custom-field-gid")
	customFieldsAddEnumOptionCmd.Flags().BoolVar(&customFieldsAddEnumOptionFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	customFieldsAddEnumOptionCmd.Flags().StringSliceVar(&customFieldsAddEnumOptionFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	customFieldsAddEnumOptionCmd.Flags().StringVar(&customFieldsAddEnumOptionFlags.data, "data", "", "")
	// Note: body fields are not MarkFlagRequired — --body JSON satisfies them too.
	customFieldsAddEnumOptionCmd.Flags().StringVar(&customFieldsAddEnumOptionFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	customFieldsCmd.AddCommand(customFieldsAddEnumOptionCmd)
}

func runCustomFieldsAddEnumOption(cmd *cobra.Command, args []string) error {
	...
	pathParams["custom_field_gid"] = fmt.Sprintf("%v", customFieldsAddEnumOptionFlags.customFieldGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/custom_fields/{custom_field_gid}/enum_options", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", customFieldsAddEnumOptionFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = customFieldsAddEnumOptionFlags.optFields
	}
	...
	if customFieldsAddEnumOptionFlags.body != "" {
		if err := json.Unmarshal([]byte(customFieldsAddEnumOptionFlags.body), &bodyMap); err != nil {
	...
	if cmd.Flags().Changed("data") {
		bodyMap["data"] = customFieldsAddEnumOptionFlags.data
	}
	req.Body = bodyMap
