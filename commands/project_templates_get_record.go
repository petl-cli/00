...
var projectTemplatesGetRecordCmd = &cobra.Command{
	Use:   "get-record",
	Short: "Get a project template",
	RunE:  runProjectTemplatesGetRecord,
}

var projectTemplatesGetRecordFlags struct {
...
func init() {
	projectTemplatesGetRecordCmd.Flags().StringVar(&projectTemplatesGetRecordFlags.projectTemplateGid, "project-template-gid", "", "Globally unique identifier for the project template.")
	projectTemplatesGetRecordCmd.MarkFlagRequired("project-template-gid")
	projectTemplatesGetRecordCmd.Flags().BoolVar(&projectTemplatesGetRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	projectTemplatesGetRecordCmd.Flags().StringSliceVar(&projectTemplatesGetRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	projectTemplatesCmd.AddCommand(projectTemplatesGetRecordCmd)
}

func runProjectTemplatesGetRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["project_template_gid"] = fmt.Sprintf("%v", projectTemplatesGetRecordFlags.projectTemplateGid)

	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/project_templates/{project_template_gid}", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", projectTemplatesGetRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = projectTemplatesGetRecordFlags.optFields
	}
