...
var teamsGetTeamRecordCmd = &cobra.Command{
	Use:   "get-team-record",
	Short: "Get a team",
	RunE:  runTeamsGetTeamRecord,
}

var teamsGetTeamRecordFlags struct {
	teamGid   string
	optPretty bool
	optFields []string
}
...
func init() {
	teamsGetTeamRecordCmd.Flags().StringVar(&teamsGetTeamRecordFlags.teamGid, "team-gid", "", "Globally unique identifier for the team.")
	teamsGetTeamRecordCmd.MarkFlagRequired("team-gid")
	teamsGetTeamRecordCmd.Flags().BoolVar(&teamsGetTeamRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	teamsGetTeamRecordCmd.Flags().StringSliceVar(&teamsGetTeamRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	teamsCmd.AddCommand(teamsGetTeamRecordCmd)
}

func runTeamsGetTeamRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["team_gid"] = fmt.Sprintf("%v", teamsGetTeamRecordFlags.teamGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", teamsGetTeamRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = teamsGetTeamRecordFlags.optFields
	}
