...
var customFieldsGetMetadataCmd = &cobra.Command{
	Use:   "get-metadata",
	Short: "Get a custom field",
	RunE:  runCustomFieldsGetMetadata,
}

var customFieldsGetMetadataFlags struct {
...
func init() {
	customFieldsGetMetadataCmd.Flags().StringVar(&customFieldsGetMetadataFlags.customFieldGid, "custom-field-gid", "", "Globally unique identifier for the custom field.")
	customFieldsGetMetadataCmd.MarkFlagRequired("custom-field-gid")
	customFieldsGetMetadataCmd.Flags().BoolVar(&customFieldsGetMetadataFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	customFieldsGetMetadataCmd.Flags().StringSliceVar(&customFieldsGetMetadataFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	customFieldsCmd.AddCommand(customFieldsGetMetadataCmd)
}

func runCustomFieldsGetMetadata(cmd *cobra.Command, args []string) error {
	...
	pathParams["custom_field_gid"] = fmt.Sprintf("%v", customFieldsGetMetadataFlags.customFieldGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", customFieldsGetMetadataFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = customFieldsGetMetadataFlags.optFields
	}
