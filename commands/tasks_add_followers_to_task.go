...
var tasksAddFollowersToTaskCmd = &cobra.Command{
	Use:   "add-followers-to-task",
	Short: "Add followers to a task",
	RunE:  runTasksAddFollowersToTask,
}

var tasksAddFollowersToTaskFlags struct {
	taskGid   string
	optPretty bool
	optFields []string
	body      string
...
func init() {
	tasksAddFollowersToTaskCmd.Flags().StringVar(&tasksAddFollowersToTaskFlags.taskGid, "task-gid", "", "The task to operate on.")
	tasksAddFollowersToTaskCmd.MarkFlagRequired("task-gid")
	tasksAddFollowersToTaskCmd.Flags().BoolVar(&tasksAddFollowersToTaskFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	tasksAddFollowersToTaskCmd.Flags().StringSliceVar(&tasksAddFollowersToTaskFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	tasksAddFollowersToTaskCmd.Flags().StringVar(&tasksAddFollowersToTaskFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	tasksCmd.AddCommand(tasksAddFollowersToTaskCmd)
}

func runTasksAddFollowersToTask(cmd *cobra.Command, args []string) error {
	...
	pathParams["task_gid"] = fmt.Sprintf("%v", tasksAddFollowersToTaskFlags.taskGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/tasks/{task_gid}/addFollowers", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", tasksAddFollowersToTaskFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = tasksAddFollowersToTaskFlags.optFields
	}
	...
	if tasksAddFollowersToTaskFlags.body != "" {
		if err := json.Unmarshal([]byte(tasksAddFollowersToTaskFlags.body), &bodyMap); err != nil {
