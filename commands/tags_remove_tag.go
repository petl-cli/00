...
var tagsRemoveTagCmd = &cobra.Command{
	Use:   "remove-tag",
	Short: "Delete a tag",
	RunE:  runTagsRemoveTag,
}

var tagsRemoveTagFlags struct {
	tagGid    string
	optPretty bool
}

func init() {
	tagsRemoveTagCmd.Flags().StringVar(&tagsRemoveTagFlags.tagGid, "tag-gid", "", "Globally unique identifier for the tag.")
	tagsRemoveTagCmd.MarkFlagRequired("tag-gid")
	tagsRemoveTagCmd.Flags().BoolVar(&tagsRemoveTagFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")

	tagsCmd.AddCommand(tagsRemoveTagCmd)
}

func runTagsRemoveTag(cmd *cobra.Command, args []string) error {
	...
	pathParams["tag_gid"] = fmt.Sprintf("%v", tagsRemoveTagFlags.tagGid)
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", tagsRemoveTagFlags.optPretty)
	}
