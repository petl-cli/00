...
var projectsRemoveCustomFieldCmd = &cobra.Command{
	Use:   "remove-custom-field",
	Short: "Remove a custom field from a project",
	RunE:  runProjectsRemoveCustomField,
}

var projectsRemoveCustomFieldFlags struct {
	projectGid string
	optPretty  bool
	body       string
}
...
func init() {
	projectsRemoveCustomFieldCmd.Flags().StringVar(&projectsRemoveCustomFieldFlags.projectGid, "project-gid", "", "Globally unique identifier for the project.")
	projectsRemoveCustomFieldCmd.MarkFlagRequired("project-gid")
	projectsRemoveCustomFieldCmd.Flags().BoolVar(&projectsRemoveCustomFieldFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	projectsRemoveCustomFieldCmd.Flags().StringVar(&projectsRemoveCustomFieldFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	projectsCmd.AddCommand(projectsRemoveCustomFieldCmd)
}

func runProjectsRemoveCustomField(cmd *cobra.Command, args []string) error {
	...
	pathParams["project_gid"] = fmt.Sprintf("%v", projectsRemoveCustomFieldFlags.projectGid)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/projects/{project_gid}/removeCustomFieldSetting", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", projectsRemoveCustomFieldFlags.optPretty)
	}
	...
	if projectsRemoveCustomFieldFlags.body != "" {
		if err := json.Unmarshal([]byte(projectsRemoveCustomFieldFlags.body), &bodyMap); err != nil {
