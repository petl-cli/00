...
var customFieldsUpdateEnumOptionCmd = &cobra.Command{
	Use:   "update-enum-option",
	Short: "Update an enum option",
	RunE:  runCustomFieldsUpdateEnumOption,
}

var customFieldsUpdateEnumOptionFlags struct {
...
func init() {
	customFieldsUpdateEnumOptionCmd.Flags().StringVar(&customFieldsUpdateEnumOptionFlags.enumOptionGid, "enum-option-gid", "", "Globally unique identifier for the enum option.")
	customFieldsUpdateEnumOptionCmd.MarkFlagRequired("enum-option-gid")
	customFieldsUpdateEnumOptionCmd.Flags().BoolVar(&customFieldsUpdateEnumOptionFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	customFieldsUpdateEnumOptionCmd.Flags().StringSliceVar(&customFieldsUpdateEnumOptionFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")
	customFieldsUpdateEnumOptionCmd.Flags().StringVar(&customFieldsUpdateEnumOptionFlags.data, "data", "", "")
	// Note: body fields are not MarkFlagRequired — --body JSON satisfies them too.
	customFieldsUpdateEnumOptionCmd.Flags().StringVar(&customFieldsUpdateEnumOptionFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	customFieldsCmd.AddCommand(customFieldsUpdateEnumOptionCmd)
}

func runCustomFieldsUpdateEnumOption(cmd *cobra.Command, args []string) error {
	...
	pathParams["enum_option_gid"] = fmt.Sprintf("%v", customFieldsUpdateEnumOptionFlags.enumOptionGid)

	req := &httpclient.Request{
		Method:      "PUT",
		Path:        httpclient.SubstitutePath("/enum_options/{enum_option_gid}", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", customFieldsUpdateEnumOptionFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = customFieldsUpdateEnumOptionFlags.optFields
	}
	...
	if customFieldsUpdateEnumOptionFlags.body != "" {
		if err := json.Unmarshal([]byte(customFieldsUpdateEnumOptionFlags.body), &bodyMap); err != nil {
	...
	if cmd.Flags().Changed("data") {
		bodyMap["data"] = customFieldsUpdateEnumOptionFlags.data
	}
	req.Body = bodyMap
