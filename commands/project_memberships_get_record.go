...
var projectMembershipsGetRecordCmd = &cobra.Command{
	Use:   "get-record",
	Short: "Get a project membership",
	RunE:  runProjectMembershipsGetRecord,
}

var projectMembershipsGetRecordFlags struct {
...
func init() {
	projectMembershipsGetRecordCmd.Flags().StringVar(&projectMembershipsGetRecordFlags.projectMembershipGid, "project-membership-gid", "", "")
	projectMembershipsGetRecordCmd.MarkFlagRequired("project-membership-gid")
	projectMembershipsGetRecordCmd.Flags().BoolVar(&projectMembershipsGetRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	projectMembershipsGetRecordCmd.Flags().StringSliceVar(&projectMembershipsGetRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	projectMembershipsCmd.AddCommand(projectMembershipsGetRecordCmd)
}

func runProjectMembershipsGetRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["project_membership_gid"] = fmt.Sprintf("%v", projectMembershipsGetRecordFlags.projectMembershipGid)

	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/project_memberships/{project_membership_gid}", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", projectMembershipsGetRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = projectMembershipsGetRecordFlags.optFields
	}
