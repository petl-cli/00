...
var sectionsListProjectSectionsCmd = &cobra.Command{
	Use:   "list-project-sections",
	Short: "Get sections in a project",
	RunE:  runSectionsListProjectSections,
}

var sectionsListProjectSectionsFlags struct {
	projectGid string
	optPretty  bool
	limit      int
	offset     string
...
func init() {
	sectionsListProjectSectionsCmd.Flags().StringVar(&sectionsListProjectSectionsFlags.projectGid, "project-gid", "", "Globally unique identifier for the project.")
	sectionsListProjectSectionsCmd.MarkFlagRequired("project-gid")
	sectionsListProjectSectionsCmd.Flags().BoolVar(&sectionsListProjectSectionsFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	sectionsListProjectSectionsCmd.Flags().IntVar(&sectionsListProjectSectionsFlags.limit, "limit", 0, "Results per page. The number of objects to return per page. The value must be between 1 and 100.")
	sectionsListProjectSectionsCmd.Flags().StringVar(&sectionsListProjectSectionsFlags.offset, "offset", "", "Offset token. An offset to the next page returned by the API. A pagination request will return an offset token, which can be used as an input parameter to the next request. If an offset is not passed in, the API will return the first page of results. *Note: You can only pass in an offset that was returned to you via a previously paginated request.*")
	sectionsListProjectSectionsCmd.Flags().StringSliceVar(&sectionsListProjectSectionsFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	sectionsCmd.AddCommand(sectionsListProjectSectionsCmd)
}

func runSectionsListProjectSections(cmd *cobra.Command, args []string) error {
	...
	pathParams["project_gid"] = fmt.Sprintf("%v", sectionsListProjectSectionsFlags.projectGid)

	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/projects/{project_gid}/sections", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", sectionsListProjectSectionsFlags.optPretty)
	}
	if cmd.Flags().Changed("limit") {
		req.QueryParams["limit"] = fmt.Sprintf("%v", sectionsListProjectSectionsFlags.limit)
	}
	if cmd.Flags().Changed("offset") {
		req.QueryParams["offset"] = fmt.Sprintf("%v", sectionsListProjectSectionsFlags.offset)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = sectionsListProjectSectionsFlags.optFields
	}
