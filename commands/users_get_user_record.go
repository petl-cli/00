...
var usersGetUserRecordCmd = &cobra.Command{
	Use:   "get-user-record",
	Short: "Get a user",
	RunE:  runUsersGetUserRecord,
}

var usersGetUserRecordFlags struct {
	userGid   string
	optPretty bool
	optFields []string
}
...
func init() {
	usersGetUserRecordCmd.Flags().StringVar(&usersGetUserRecordFlags.userGid, "user-gid", "", "A string identifying a user. This can either be the string \"me\", an email, or the gid of a user.")
	usersGetUserRecordCmd.MarkFlagRequired("user-gid")
	usersGetUserRecordCmd.Flags().BoolVar(&usersGetUserRecordFlags.optPretty, "opt-pretty", false, "Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.")
	usersGetUserRecordCmd.Flags().StringSliceVar(&usersGetUserRecordFlags.optFields, "opt-fields", nil, "This endpoint returns a compact resource, which excludes some properties by default. To include those optional properties, set this query parameter to a comma-separated list of the properties you wish to include.")

	usersCmd.AddCommand(usersGetUserRecordCmd)
}

func runUsersGetUserRecord(cmd *cobra.Command, args []string) error {
	...
	pathParams["user_gid"] = fmt.Sprintf("%v", usersGetUserRecordFlags.userGid)

	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/users/{user_gid}", pathParams),
	...
	if cmd.Flags().Changed("opt-pretty") {
		req.QueryParams["opt_pretty"] = fmt.Sprintf("%v", usersGetUserRecordFlags.optPretty)
	}
	if cmd.Flags().Changed("opt-fields") {
		req.ArrayParams["opt_fields"] = usersGetUserRecordFlags.optFields
	}
